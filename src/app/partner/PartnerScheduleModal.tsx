...
export default function PartnerScheduleModal({ orgId, orgType, onClose, onCreated, initialDate = "" }: Props) {
  ...
  }, [bookingType, vehicleType, dayType, numStops, deliveryType, zone, selectedServices, stairFlights, isAfterHours, isWeekend]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 backdrop-blur-sm p-0 sm:p-4" onClick={onClose}>
      <div
        className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-[640px] max-h-[92vh] overflow-y-auto mx-0 sm:mx-4 flex flex-col"
            ...
            <div className="space-y-5">
              ...
              <section className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[20px] font-bold text-[#1A1A1A] w-10 text-center">{numStops}</span>
            ...
            <div className="space-y-5">
              ...
              <section className="space-y-2">
                ...
                <select value={zone} onChange={(e) => setZone(Number(e.target.value))} className={fieldInput}>
                  <option value={1}>Zone 1 — GTA Core (0-15 km)</option>
                  <option value={2}>Zone 2 — Inner Suburbs (15-30 km)</option>
                  <option value={3}>Zone 3 — Outer GTA (30-50 km)</option>
                  <option value={4}>Zone 4 — Extended (50-80 km)</option>
                  <option value={5}>Zone 5 — Remote (80+ km, quote on request)</option>
                </select>
              </section>
    ...
    return (
      <div className="rounded-xl border border-[#C9A962]/30 bg-[#FFFDF7] p-4 space-y-2">
        ...
        {pricing ? (
          ...
          <div className="text-[12px] text-[#888]">{pricingLoading ? "Loading rates…" : "Configure options above to see pricing"}</div>
        )}
      </div>
    );
  }
