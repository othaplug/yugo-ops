...
export default function PartnerShareModal({ delivery, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[var(--card)] rounded-2xl shadow-xl w-full max-w-[420px] mx-4" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-h3-lg font-bold text-[var(--tx)] font-serif">Share Tracking Link</h2>
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-[var(--bg)] transition-colors">
          ...
          <div className="bg-[var(--bg)] rounded-xl p-3 mb-4">
            <div className="text-body font-semibold text-[var(--tx)]">{delivery.customer_name || delivery.delivery_number}</div>
            <div className="text-caption text-[var(--tx3)] mt-0.5">{delivery.delivery_address || "—"}</div>
          </div>

          {error && (
            <div className="mb-3 px-3 py-2 rounded-lg bg-red-50 border border-red-200 text-ui text-red-700">{error}</div>
          )}
          ...
          {sent ? (
            <div className="text-center py-6">
              ...
              <p className="text-title font-semibold text-[var(--tx)]">Tracking link sent!</p>
            </div>
          ) : (
            <>
              <div className="flex gap-2 mb-4">
                <button
                  ...
                  className={`flex-1 px-3 py-2 rounded-lg text-ui font-semibold border transition-colors ${
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-lg border border-[var(--brd)] text-body text-[var(--tx)] placeholder-[var(--tx3)] focus:border-[#C9A962] focus:outline-none transition-colors mb-4"
              />

              <button
                onClick={handleSend}
                disabled={sending}
                className="w-full px-4 py-2.5 rounded-lg text-body font-bold bg-[#C9A962] text-white hover:bg-[#B89A52] transition-colors disabled:opacity-50"
              >
