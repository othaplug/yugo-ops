  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 bg-[#FAF8F5]">
      <div className="max-w-md w-full text-center space-y-5">
        ...
        <h1 className="text-h3-lg font-bold text-[#1A1714] font-serif">Something went wrong</h1>
        <p className="text-body text-[#888]">{error.message || "An unexpected error occurred."}</p>
        <div className="flex gap-3 justify-center">
          <button onClick={reset} className="px-4 py-2 rounded-lg text-ui font-semibold bg-[#2D6A4F] text-white">Try again</button>
          <Link href="/partner" className="px-4 py-2 rounded-lg text-ui font-semibold border border-[#E8E4DF] text-[#1A1714]">Dashboard</Link>
        </div>
      </div>
    </div>
  );
