    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-h3-lg font-bold text-[var(--tx)] font-hero">Service Level Performance</h3>
          <button
      ...
      <div>
        <h3 className="text-h3 font-bold text-[var(--tx)] font-hero mb-3">Monthly Performance</h3>
      ...
      {/* Industry Comparison */}
      <div>
        <h3 className="text-h3 font-bold text-[var(--tx)] font-hero mb-3 flex items-center gap-2">
      ...
      {/* Revenue breakdown */}
      <div>
        <h3 className="text-h3 font-bold text-[var(--tx)] font-hero mb-3">Revenue Summary</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
            ...
            <div className="text-h2 font-bold text-[var(--tx)] mt-1 font-hero">{formatCurrency(totalRevenue)}</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
            <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)]">Paid</div>
            <div className="text-h2 font-bold text-[#2D9F5A] mt-1 font-hero">{formatCurrency(totalPaid)}</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
            <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)]">Outstanding</div>
            <div className="text-h2 font-bold text-[var(--red)] mt-1 font-hero">{formatCurrency(data.outstandingAmount)}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-5 flex flex-col items-center text-center">
      <div className={`w-[80px] h-[80px] rounded-full border-4 flex items-center justify-center ${accent ? "border-[var(--gold)]" : "border-[var(--brd)]"}`}>
        <span className={`text-h3 font-bold ${accent ? "text-[var(--gold)]" : "text-[var(--tx)]"} font-hero`}>{value}</span>
      </div>
  ...
  return (
    <div className={`bg-[var(--card)] border rounded-xl p-4 ${accent ? "border-l-2 border-l-[var(--gold)] border-[var(--brd)]" : "border-[var(--brd)]"}`}>
      ...
      <div className="flex items-baseline justify-between mt-2">
        <div>
          <span className="text-h2 font-bold text-[var(--tx)] font-hero">{yugoValue}</span>
