    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[26px] font-bold text-[var(--tx)] font-hero">Service Level Performance</h3>
          <button
      ...
      <div>
        <h3 className="text-[26px] font-bold text-[var(--tx)] font-hero mb-3">Monthly Performance</h3>
        <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--brd)]">
                  <th className="px-4 py-3 text-left text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Month</th>
                  <th className="px-4 py-3 text-left text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Deliveries</th>
                  <th className="px-4 py-3 text-left text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Completed</th>
                  <th className="px-4 py-3 text-left text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">On-Time</th>
                  <th className="px-4 py-3 text-left text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Damage</th>
                  <th className="px-4 py-3 text-right text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Score</th>
                </tr>
              </thead>
              <tbody>
                {monthlyData.map((row) => (
                  <tr key={row.month} className="border-b border-[var(--brd)] last:border-0">
                    <td className="px-4 py-3 text-[13px] font-semibold text-[var(--tx)]">{row.month}</td>
                    <td className="px-4 py-3 text-[13px] text-[var(--tx)]">{row.deliveries}</td>
                    <td className="px-4 py-3 text-[13px] text-[var(--tx)]">{row.completed}</td>
                    <td className="px-4 py-3 text-[13px] font-semibold text-[var(--tx)]">{row.onTime}</td>
                    <td className="px-4 py-3 text-[13px] text-[var(--tx)]">{row.damage}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={`text-[12px] font-bold ${row.score.startsWith("A") ? "text-[#2D9F5A]" : "text-[var(--gold)]"}`}>{row.score}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {/* Revenue breakdown */}
      <div>
        <h3 className="text-[26px] font-bold text-[var(--tx)] font-hero mb-3">Revenue Summary</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
            ...
            <div className="text-[30px] font-bold text-[var(--tx)] mt-1 font-hero">{formatCurrency(totalRevenue)}</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
            <div className="text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Paid</div>
            <div className="text-[30px] font-bold text-[#2D9F5A] mt-1 font-hero">{formatCurrency(totalPaid)}</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
            <div className="text-[10px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Outstanding</div>
            <div className="text-[30px] font-bold text-[var(--red)] mt-1 font-hero">{formatCurrency(data.outstandingAmount)}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-5 flex flex-col items-center text-center">
      <div className={`w-[80px] h-[80px] rounded-full border-4 flex items-center justify-center ${accent ? "border-[var(--gold)]" : "border-[var(--brd)]"}`}>
        <span className={`text-[26px] font-bold ${accent ? "text-[var(--gold)]" : "text-[var(--tx)]"} font-hero`}>{value}</span>
      </div>
      <div className="text-[11px] font-semibold text-[var(--tx)] mt-3">{label}</div>
  ...
  return (
    <div className={`bg-[var(--card)] border rounded-xl p-4 ${accent ? "border-l-2 border-l-[var(--gold)] border-[var(--brd)]" : "border-[var(--brd)]"}`}>
      ...
      <div className="flex items-baseline justify-between mt-2">
        <div>
          <span className="text-[30px] font-bold text-[var(--tx)] font-hero">{yugoValue}</span>
