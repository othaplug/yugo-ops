  ...
  if (projects.length === 0) {
    return (
      <div className="bg-white border border-[#E8E4DF] rounded-xl p-8 text-center">
        <p className="text-title text-[#888]">No active projects.</p>
      </div>
    );
  }
          ...
          <div key={project.id} className="bg-white border border-[#E8E4DF] rounded-xl overflow-hidden hover:border-[#C9A962]/40 transition-colors">
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-h3-md font-bold text-[#1A1A1A] font-serif">{project.name}</h3>
                ...
                <div className="flex items-center gap-2">
                  {hasDelay && (
                    <span className="px-2.5 py-1 rounded-full text-label font-semibold bg-red-50 text-red-600 border border-red-200">
                  ...
                  {onShareProject && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-caption font-semibold border border-[#E8E4DF] text-[#888] hover:border-[#C9A962] hover:text-[#C9A962] transition-colors"
                    >
              ...
              <div className="text-right text-caption text-[#888] mt-1 font-medium">{project.percent}%</div>
            </div>

            {/* Vendor List (expanded) */}
            {isExpanded && (
              <div className="border-t border-[#E8E4DF] px-5 py-3">
                <div className="text-label font-semibold tracking-wider uppercase text-[#888] mb-3">Vendor Receiving</div>
                    ...
                    return (
                      <div key={`${v.vendor}-${i}`} className="flex items-center justify-between py-1">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-body font-semibold text-[#1A1A1A]">{v.vendor}</span>
                            <span className="text-ui text-[#888] ml-1.5">— {v.items}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`text-label font-bold tracking-wider ${colorClass}`}>
                            {label} {v.eta}
                          </span>
                        </div>
                      </div>
