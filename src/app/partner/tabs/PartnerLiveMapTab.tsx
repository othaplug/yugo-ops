      ...
      <div className={`relative rounded-xl border border-[#E8E4DF] overflow-hidden bg-white ${isFullscreen ? "fixed inset-0 z-50 rounded-none" : ""}`} style={isFullscreen ? undefined : { height: 480 }}>
        ...
        {crewHasStarted && selected && (
          <div className="absolute top-4 left-4 z-10 bg-white rounded-xl border border-[#E8E4DF] p-4 shadow-lg max-w-[280px]">
            ...
            <button onClick={() => setSelected(null)} className="mt-2 text-[10px] text-[#2C3E2D] font-semibold hover:underline">
              Close
            </button>
          </div>
        )}
        ...
        {!crewHasStarted && crewIsOutWorking && currentDelivery && (
          <div className="absolute top-4 left-4 z-10 bg-white rounded-xl border border-amber-200 p-3.5 shadow-lg max-w-[280px]">
            ...
            {currentDelivery.crew_name && (
              <div className="text-[11px] text-[#4F4B47] mt-1">Team: {currentDelivery.crew_name}</div>
            )}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-center p-8 text-[#D4D0CB]">
            <p className="text-[var(--text-base)] text-[#4F4B47]">No Confirmed Deliveries Scheduled For Today.</p>
          ...
          <>
            ...
            {!crewHasStarted && !crewIsOutWorking && (
              <div className="absolute inset-0 bg-white/70 backdrop-blur-md flex flex-col items-center justify-center z-10" aria-hidden="true">
                ...
                <span className="text-[12px] text-[#454545] mt-1">Map and tracking will appear when crew start your job</span>
              </div>
            )}
          </>
        )}
            ...
            return (
              <div
                ...
                className={`bg-white border rounded-xl p-4 flex items-center justify-between transition-colors ${
                  isActiveDelivery
                    ? "border-[#2C3E2D] ring-1 ring-[#2C3E2D]/30"
                    : "border-[#E8E4DF]"
                } ${isActive ? "cursor-pointer hover:border-[#2C3E2D]/40" : ""}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-[var(--text-base)] font-semibold text-[#1A1A1A]">
                      {d.customer_name || d.delivery_number}
                    </div>
                    {isActive ? (
                      <div className="text-[12px] text-[#4F4B47] mt-0.5">
                        {d.delivery_address || "-"}
                      </div>
                    ) : crewIsOutWorking ? (
                ...
                <div className="flex items-center gap-2">
                  {isActive && d.crew_name && (
                    <span className="text-[11px] text-[#4F4B47]">{d.crew_name}</span>
                  )}
                  {isActive ? (
