...
export async function GET(
  ...
  const { data: self } = await admin
    .from("moves")
    .select("id, partner_property_id, unit_number, tenant_name, is_pm_move")
    .or(`id.eq.${id},move_code.eq.${id}`)
    .maybeSingle();
  if (!self) return NextResponse.json({ candidates: [] });

  // Candidate scoring: prefer same building, then same tenant_name token,
  // then most-recent PM moves. The list never auto-applies — the admin
  // confirms the pairing in the UI.
  let query = admin
    .from("moves")
    .select(
      "id, move_code, scheduled_date, status, partner_property_id, unit_number, tenant_name, pm_reason_code, from_address, to_address, linked_move_id",
    )
    .neq("id", self.id)
