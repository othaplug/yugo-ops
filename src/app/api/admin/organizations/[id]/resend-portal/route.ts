...
export async function POST(
  ...
  try {
    ...
    if (sendError) {
      ...
      return NextResponse.json(
        { error: message.includes("domain") || message.includes("verified") ? `Email failed: ${message}. Verify the sending domain for your notification address in Resend.` : message },
        { status: 500 }
      );
    }
