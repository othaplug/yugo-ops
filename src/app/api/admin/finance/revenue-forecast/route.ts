...
function mergeMovesById<T extends { id: string }>(a: T[], b: T[]): T[] {
...
/** Quote counts toward a forward-looking window when move_date is in range, or when undated (still open). */
function quoteInPipelineWindow(q: PipelineQuoteRow, todayStr: string, endStr: string): boolean {
...
export async function GET(_req: NextRequest) {
  ...
  const forecasts = await Promise.all(
    periods.map(async (days) => {
      ...
      if (scheduledMovesRes.error) console.error("[revenue-forecast] moves (scheduled):", scheduledMovesRes.error.message);
      if (unscheduledMovesRes.error) {
        console.error("[revenue-forecast] moves (unscheduled):", unscheduledMovesRes.error.message);
      }
      if (deliveriesRes.error) console.error("[revenue-forecast] deliveries:", deliveriesRes.error.message);

      const confirmedMoves = mergeMovesById(scheduledMovesRes.data ?? [], unscheduledMovesRes.data ?? []);
