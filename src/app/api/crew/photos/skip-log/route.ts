...
const PHOTO_SKIP_STAGE_LABEL: Record<string, string> = {
  en_route_to_pickup: "en route to pickup",
  arrived_at_pickup: "pickup",
  loading: "loading",
  en_route_to_destination: "en route to destination",
  arrived_at_destination: "destination",
  unloading: "unloading",
  completed: "completed",
  en_route: "en route",
  arrived: "arrived",
...
function humanizePhotoSkipStage(raw: string): string {
  const k = raw.trim().toLowerCase();
  if (PHOTO_SKIP_STAGE_LABEL[k]) return PHOTO_SKIP_STAGE_LABEL[k];
  return k.replace(/_/g, " ").trim() || "this stage";
}
...
export async function POST(req: NextRequest) {
  ...
  if (!jobId || !["move", "delivery"].includes(jobType)) {
    return NextResponse.json(
      { error: "jobId and jobType required" },
      { status: 400 },
    );
  }
  ...
  const { data: crewRow } = await admin
    ...
    .maybeSingle();
  const teamName = String(crewRow?.name ?? "").trim() || "Crew";
  const stageLabel = humanizePhotoSkipStage(checkpoint || "");
  ...
  try {
    await admin.from("status_events").insert({
      ...
      description: `${teamName} skipped mandatory photos at ${stageLabel}`,
      icon: "camera",
    });
  } catch {}
