...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: phases }, { data: inventory }, { data: timeline }, { data: deliveries }] = await Promise.all([
    ...
    db.from("project_inventory").select("id, phase_id, item_name, vendor, quantity, status, received_date, condition_on_receipt, photo_urls, storage_location").eq("project_id", id).order("created_at"),
