...
export default async function AdminPage() {
  ...
  const cutoff90 = new Date(Date.now() - 90 * 24 * 3600_000)
    .toISOString()
    .slice(0, 10);
    ...
    (async () => {
      try {
        ...
        const avgSec =
          avgRows.length > 0
            ? avgRows.reduce(
                (s, l) => s + (Number(l.response_time_seconds) || 0),
                0,
              ) / avgRows.length
            : null;
  ...
  const changeTasks: ActionTask[] = (pendingChangesResult?.data || []).map(
    (r: Record<string, unknown>) => {
      ...
      const move = Array.isArray(moveRaw)
        ? ((moveRaw[0] as
            | { client_name?: string; move_code?: string }
            | undefined) ?? null)
      ...
      return {
        ...
        subtitle: [move?.client_name, move?.move_code]
          .filter(Boolean)
          .join(" · "),
  ...
  const todayMoves = allMoves.filter(
  ...
  if (todayTotal > 0) {
    ...
    if (dlvCount > 0)
      parts.push(`${dlvCount} deliver${dlvCount > 1 ? "ies" : "y"}`);
    briefParts.push(
  ...
  if (unassignedJobs.length > 0) {
    const todayUnassigned = unassignedJobs.filter(
      (j) => j.date === today,
    ).length;
