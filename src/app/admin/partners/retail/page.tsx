    ...
    <div className="max-w-[1200px] mx-auto px-5 md:px-6 py-5">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-lg p-3">
            ...
            <div className="text-xl font-bold font-serif">{clients.length}</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-lg p-3">
            <div className="text-[9px] font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Deliveries</div>
            <div className="text-xl font-bold font-serif">{dels.length}</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-lg p-3">
            <div className="text-[9px] font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">On-Time</div>
            <div className="text-xl font-bold font-serif text-[var(--grn)]">100%</div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-lg p-3">
            <div className="text-[9px] font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Revenue</div>
            <div className="text-xl font-bold font-serif">$17.3K</div>
          </div>
        </div>

        {/* Partners */}
        <h3 className="text-[13px] font-bold mb-2">{clients.length} Partners</h3>
        {clients.map((c) => (
          <div key={c.id} className="bg-[var(--card)] border border-[var(--brd)] rounded-lg p-3 mb-1.5 hover:border-[var(--gold)] transition-all cursor-pointer">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] font-semibold">{c.deliveries_per_month} del/mo</span>
        ...
        <div className="flex flex-col gap-1">
          {dels.slice(0, 5).map((d) => (
            <div key={d.id} className="flex items-center gap-2.5 px-3 py-2.5 bg-[var(--card)] border border-[var(--brd)] rounded-lg hover:border-[var(--gold)] transition-all">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm bg-[var(--gdim)]">🛋️</div>
