...
export default async function HospitalityPage() {
  ...
  const hospitalityInvoices = (invoices || []).filter(
    (i) => i.client_name && hospitalityNames.has(i.client_name),
  );
  const paid = hospitalityInvoices.filter((i) => i.status === "paid");
  const outstanding = hospitalityInvoices.filter(
