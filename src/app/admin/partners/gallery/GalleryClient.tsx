...
export default function GalleryClient({ galleryPartners = [] }: { galleryPartners?: GalleryPartner[] }) {
  ...
  if (loading) {
    return (
      <div className="mb-6 py-8 text-center text-ui text-[var(--tx3)]">Loading projects…</div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="pt-6 border-t border-[var(--brd)]/30">
        <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-4">Projects</div>
        <div className="space-y-0 divide-y divide-[var(--brd)]/30">
          {allProjects.length === 0 ? (
            <div className="py-8 text-center text-ui text-[var(--tx3)]">
              No projects yet
            </div>
          ) : allProjects.map((ex) => {
              ...
              <div key={ex.id} className="py-3 first:pt-0">
                <button type="button" onClick={toggle} className="w-full flex items-center gap-3 px-4 py-3 text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-ui font-bold text-[var(--tx)]">{ex.name}</div>
                    <div className="text-label text-[var(--tx3)] mt-0.5">{subtitle}</div>
                  </div>
                  <div className="text-right shrink-0">
                    {!isExhibition && ex.start_date && <div className="text-label text-[var(--tx3)]">{formatDateShort(ex.start_date)}</div>}
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-0 border-t border-[var(--brd)]/30">
                    <div className="text-caption text-[var(--tx2)] mt-3 leading-relaxed">{ex.details || "—"}</div>
                    <button type="button" onClick={() => openDetail(ex)} className="inline-block mt-2 text-label font-semibold text-[var(--gold)] hover:underline mr-3">View details →</button>
                    <button type="button" onClick={() => openEdit(ex)} className="inline-block mt-2 text-label font-semibold text-[var(--tx2)] hover:underline">Edit project</button>
                  </div>
                )}
              </div>
            );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" aria-modal="true">
          ...
          <div className="relative bg-[var(--card)] border border-[var(--brd)] rounded-xl w-full max-w-md p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3 mb-4">
              <h3 className="font-heading text-h3-sm font-bold text-[var(--tx)]">Project details</h3>
            ...
            <div className="space-y-3 text-ui">
              <div>
                <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-0.5">Name</div>
                <div className="text-[var(--tx)] font-semibold">{projectDetail.name}</div>
              </div>
              <div>
                <div className="text-section font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">Gallery</div>
                <div className="text-[var(--tx)]">{projectDetail.gallery || "—"}</div>
              </div>
              <div>
                <div className="text-section font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">Type</div>
                <div className="text-[var(--tx)]">{toTitleCase(projectDetail.project_type || "—")}</div>
              </div>
              <div>
                <div className="text-section font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">Address</div>
                <div className="text-[var(--tx)]">{projectDetail.address || "—"}</div>
              </div>
              <div>
                <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-0.5">Location</div>
                <div className="text-[var(--tx)]">{projectDetail.location || "—"}</div>
              </div>
              <div>
                <div className="text-section font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">Dates</div>
              ...
              <div>
                <div className="text-section font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">Estimate</div>
                <div className="text-[var(--gold)] font-semibold">{projectDetail.insurance_value || "—"}</div>
              </div>
              <div>
                <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-0.5">Status</div>
              ...
              {(projectDetail.white_glove || projectDetail.crating_required || projectDetail.climate_controlled) && (
                <div>
                  <div className="text-section font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">Handling</div>
              ...
              <div>
                <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-0.5">Details</div>
                <p className="text-[var(--tx2)] leading-relaxed">{projectDetail.details || "—"}</p>
              </div>
            </div>
            <button type="button" onClick={() => openEdit(projectDetail)} className="mt-4 w-full py-2.5 rounded-lg text-caption font-semibold bg-[var(--gold)] text-[var(--btn-text-on-accent)] hover:bg-[var(--gold2)]">
              Edit project
            </button>
          </div>
        </div>
