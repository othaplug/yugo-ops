  ...
  return (
    <div className="max-w-[1200px] mx-auto px-5 md:px-6 py-5 md:py-6 animate-fade-up">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="font-heading text-h2 font-bold text-[var(--tx)]">Gallery Partners</h1>
          <p className="text-ui text-[var(--tx3)] mt-0.5">{galleryPartners.length} active partner{galleryPartners.length !== 1 ? "s" : ""} · {projectCount} projects</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-6">
        <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-h1 font-bold font-heading text-[var(--tx)]">{galleryPartners.length}</span>
            <StatPctChange current={galleryPartners.length} previous={partnersPrev} />
          </div>
        </div>
        <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
          <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Projects</div>
          <div className="flex items-baseline gap-2">
            <span className="text-h1 font-bold font-heading text-[var(--tx)]">{projectCount}</span>
            <StatPctChange current={projectsThisMonth} previous={projectsLastMonth} />
          </div>
        </div>
        <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4">
          <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Revenue</div>
          <div className="flex items-baseline gap-2">
            <span className="text-h1 font-bold font-heading text-[var(--grn)]">{formatCompactCurrency(revenueTotal)}</span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        <h3 className="font-heading text-title font-bold text-[var(--tx)]">Projects & Partners</h3>
        <div className="flex gap-2">
          <Link href="/admin/deliveries/new?type=gallery" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-caption font-semibold border border-[var(--brd)] text-[var(--tx)] hover:border-[var(--gold)] bg-[var(--card)] transition-all">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            Create Project
          </Link>
          <Link href="/admin/clients/new?type=partner&partnerType=gallery" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-caption font-semibold bg-[var(--gold)] text-[var(--btn-text-on-accent)] hover:bg-[var(--gold2)] transition-all">
      ...
      {/* Partners list */}
      <div className="mt-6">
        <h3 className="font-heading text-body font-bold text-[var(--tx)] mb-3">Partners</h3>
        <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl overflow-hidden divide-y divide-[var(--brd)]/50">
          {galleryPartners.length === 0 ? (
            ...
            <Link key={p.id} href={`/admin/clients/${p.id}?from=gallery`} className="flex items-center gap-3 px-4 py-3.5 hover:bg-[var(--bg)]/50 transition-colors">
              <div className="w-9 h-9 rounded-lg bg-[#4A7CE5]/10 flex items-center justify-center text-ui font-bold text-[#4A7CE5]">
                {(p.name || "?").charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-body font-semibold text-[var(--tx)]">{p.name}</div>
                <div className="text-caption text-[var(--tx3)]">{p.contact_name || p.email || "—"}</div>
              </div>
