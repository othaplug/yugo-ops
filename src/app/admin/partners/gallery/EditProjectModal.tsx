    ...
    <ModalOverlay open={open} onClose={onClose} title="Edit project" maxWidth="md">
      <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 rounded-lg text-[11px] font-semibold bg-[var(--gold)] text-white hover:bg-[var(--gold2)] transition-all disabled:opacity-50">{loading ? "Saving…" : "Save changes"}</button>
        </div>
      </form>
    </ModalOverlay>
  );
