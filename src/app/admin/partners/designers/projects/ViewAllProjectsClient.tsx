    ...
    <>
      ...
      <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl overflow-hidden">
        <div className="divide-y divide-[var(--brd)]">
          {list.length === 0 ? (
            <div className="px-4 py-12 text-center text-ui text-[var(--tx3)]">No projects</div>
          ) : list.map((project) => {
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1">
                  <h4 className="font-heading text-body font-bold text-[var(--tx)] truncate">{project.name}</h4>
                  ...
                  <div className="text-label text-[var(--tx2)] mt-1">{summary}</div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className={`text-caption font-bold ${project.percent === 100 ? "text-[var(--grn)]" : "text-[var(--gold)]"}`}>
                    {project.percent}%
                  </span>
                  <span className="text-label font-medium text-[var(--gold)] group-hover:underline">
                    View details →
                  </span>
                </div>
              </Link>
