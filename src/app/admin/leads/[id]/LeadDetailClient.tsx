...
export default function LeadDetailClient({
  ...
  return (
    <div className="max-w-[720px] mx-auto px-4 md:px-6 py-5 md:py-6">
      ...
      {Boolean(
        ...
        <section className="rounded-xl border border-[var(--brd)] bg-[var(--card)] p-4 mb-6 space-y-2 text-[12px] text-[var(--tx2)]">
          ...
          <p>
            ...
            {COMPLETENESS_PATH_LABELS[String(lead.completeness_path)] || String(lead.completeness_path || "—")}
            {lead.completeness_score != null ? ` · Score ${String(lead.completeness_score)}` : ""}
          </p>
          {Array.isArray(lead.fields_present) && (lead.fields_present as unknown[]).length > 0 ? (
            <p>
              <span className="text-[var(--tx3)]">Present:</span> {(lead.fields_present as string[]).join(", ")}
            </p>
          ) : null}
          {Array.isArray(lead.fields_missing) && (lead.fields_missing as unknown[]).length > 0 ? (
            <p>
              <span className="text-[var(--tx3)]">Missing:</span> {(lead.fields_missing as string[]).join(", ")}
            </p>
          ) : null}
          {Array.isArray(lead.clarifications_needed) && (lead.clarifications_needed as unknown[]).length > 0 ? (
            <p>
              <span className="text-[var(--tx3)]">Clarifications:</span>{" "}
              {(lead.clarifications_needed as string[]).join(" · ")}
            </p>
          ) : null}
          {Array.isArray(lead.follow_up_questions) && (lead.follow_up_questions as unknown[]).length > 0 ? (
            <div>
              <span className="text-[var(--tx3)]">Follow-up questions sent:</span>
          ...
          {Array.isArray(lead.detected_dates) && (lead.detected_dates as unknown[]).length > 0 ? (
            <p>
              <span className="text-[var(--tx3)]">Dates in text:</span> {(lead.detected_dates as string[]).join(", ")}
            </p>
          ) : null}
          {lead.external_platform ? (
            <p>
              <span className="text-[var(--tx3)]">External platform:</span> {String(lead.external_platform)}
              {lead.external_reference ? ` · Ref ${String(lead.external_reference)}` : ""}
            </p>
          ) : null}
          {lead.raw_inquiry_text ? (
            <div className="mt-2">
      ...
      <section className="rounded-xl border border-[var(--brd)] bg-[var(--card)] p-4 mb-6 space-y-2 text-[12px] text-[var(--tx2)]">
        <p>
          <span className="text-[var(--tx3)]">Source:</span> {sourceLabel(source, detail)}
        </p>
        <p>
        ...
        <p>
          <span className="text-[var(--tx3)]">Service:</span> {String(lead.service_type || "—")}
        </p>
        <p>
          <span className="text-[var(--tx3)]">Move size:</span> {String(lead.move_size || "—")}
        </p>
        <p>
          <span className="text-[var(--tx3)]">Preferred date:</span> {String(lead.preferred_date || "—")}
        </p>
        <p>
          <span className="text-[var(--tx3)]">From:</span> {String(lead.from_address || "—")}
        </p>
        <p>
          <span className="text-[var(--tx3)]">To:</span> {String(lead.to_address || "—")}
        </p>
        {lead.message ? (
          <p>
            <span className="text-[var(--tx3)]">Message:</span> {String(lead.message)}
          </p>
        ) : null}
        {lead.quote_uuid ? (
          <p>
      ...
      <section className="mb-6">
        <label className="block text-[11px] font-bold text-[var(--tx3)] mb-1">Update status</label>
        <select
      ...
      <section className="mb-6">
        <label className="block text-[11px] font-bold text-[var(--tx3)] mb-1">Add note</label>
        <textarea
          value={note}
        ...
        <ul className="space-y-3">
          {activities.map((a) => (
            <li key={a.id} className="border-l-2 border-[var(--gold)]/40 pl-3 text-[12px] text-[var(--tx2)]">
              ...
              <div className="text-[10px] text-[var(--tx3)]">{new Date(a.created_at).toLocaleString()}</div>
