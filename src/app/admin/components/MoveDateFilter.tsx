...
export default function MoveDateFilter({ value, onChange, className = "", label = "Move date" }: MoveDateFilterProps) {
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <span className="text-label font-medium text-[var(--tx3)] shrink-0">{label}</span>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="text-caption bg-[var(--card)] border border-[var(--brd)] rounded-md px-2.5 py-1.5 text-[var(--tx)] focus:border-[var(--gold)] outline-none min-w-[120px] min-h-[36px]"
      >
