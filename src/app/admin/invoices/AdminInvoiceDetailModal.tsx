...
export default function AdminInvoiceDetailModal({
  ...
  const delivery = detail?.delivery as
  ...
  return (
    <ModalOverlay
      open={open}
      onClose={onClose}
      title={`Invoice ${displayNum || invoice.invoice_number}`}
      maxWidth="lg"
    >
      <div className="p-5 space-y-5">
        {loading ? (
          <div className="py-12 text-center text-[var(--tx3)] text-[13px]">
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-lg bg-[var(--bg)]/50 border border-[var(--brd)]/30">
              <div>
                <div className="text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">
                  Client
                </div>
                <div className="text-[var(--tx)] font-semibold">
                  {String(inv.client_name ?? invoice.client_name ?? "-")}
                </div>
              </div>
              <div>
                <div className="text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">
              ...
              <div>
                ...
                <div className="text-[var(--tx)] font-bold">
                  {formatCurrency(subtotal)}
                </div>
                <div className="text-[10px] text-[var(--tx3)]">
                  +{formatCurrency(hst)} HST · Total {formatCurrency(total)}
                </div>
              </div>
              <div>
                <div className="text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-0.5">
            ...
            {delivery && (
              <div className="p-4 rounded-lg bg-[var(--bg)]/30 border border-[var(--brd)]/20">
                <div className="text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-2">
                  Delivery
                </div>
                <div className="space-y-1 text-[12px]">
                  {!!delivery.delivery_number && (
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-semibold text-[var(--gold)]">
                        #{String(delivery.delivery_number)}
                      </span>
                      <Link
                        href={getDeliveryDetailPath({
                  ...
                  {!!delivery.delivery_address && (
                    <div className="text-[var(--tx2)]">
                      <span className="text-[var(--tx3)]">Address:</span>{" "}
                      {String(delivery.delivery_address)}
                    </div>
                  )}
                  {!!delivery.scheduled_date && (
                    <div className="text-[var(--tx2)]">
                      <span className="text-[var(--tx3)]">Scheduled:</span>{" "}
                      {String(delivery.scheduled_date)}
                      {!!delivery.time_slot &&
                        ` · ${String(delivery.time_slot)}`}
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <table className="w-full text-[12px]">
                    <thead>
                      <tr className="bg-[var(--bg)]/50 border-b border-[var(--brd)]/30">
                        <th className="text-left py-2 px-3 font-semibold text-[var(--tx3)]">
                          Description
                        </th>
                        <th className="text-right py-2 px-3 font-semibold text-[var(--tx3)]">
                          Qty
                        </th>
                        <th className="text-right py-2 px-3 font-semibold text-[var(--tx3)]">
                          Rate
                      ...
                      {lineItems.map((item, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-2 px-3 text-right text-[var(--tx2)]">
                              {q}
                            </td>
                            <td className="py-2 px-3 text-right text-[var(--tx2)]">
                              {formatCurrency(r)}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="mt-1 text-[10px] text-[var(--tx3)]">
                        +{formatCurrency(calcHST(amount || 0))} HST (13%) ·
                        Total{" "}
                        {formatCurrency(
                          Number(amount || 0) + calcHST(amount || 0),
                        )}
                      </div>
                    </div>
                    <div>
