    ...
    <>
      ...
      <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4 mb-3">
        <div
          ...
          style={{ height: 350, filter: "saturate(0.2) brightness(0.7)" }}
        />
      </div>
      ...
      <div className="flex flex-col gap-1">
        {crews.map((c) => (
          <div
            key={c.id}
            className="flex items-center gap-2.5 px-3 py-2.5 bg-[var(--card)] border border-[var(--brd)] rounded-lg"
          >
            ...
            <div className={`w-2 h-2 rounded-full ${
              c.status === "en-route" ? "bg-[var(--org)]" : "bg-[var(--grn)]"
            }`} />
          </div>
        ))}
      </div>
