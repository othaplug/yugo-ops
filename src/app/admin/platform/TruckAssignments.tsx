  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-[var(--brd)] bg-[var(--bg2)] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="font-heading text-title font-bold text-[var(--tx)] flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <input
            ...
            className="px-3 py-1.5 rounded-lg bg-[var(--bg)] border border-[var(--brd)] text-ui text-[var(--tx)] focus:border-[var(--gold)] outline-none"
          />
          <button
            onClick={() => setAddModalOpen(true)}
            disabled={trucks.length === 0 || teams.length === 0}
            className="px-3 py-1.5 rounded-lg text-label font-semibold border border-[var(--brd)] text-[var(--tx2)] hover:border-[var(--gold)] hover:text-[var(--gold)] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            + Assign
          </button>
        </div>
      ...
      <div className="px-5 py-4">
        {loading ? (
          ...
          <>
            <div className="text-caption text-[var(--tx3)] mb-3">{formatDateLabel(date)}</div>
            {assignments.length === 0 ? (
              <div className="py-6 text-center">
                <p className="text-body text-[var(--tx3)] mb-1">No assignments for this date</p>
                ...
                {assignments.map((a) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-body font-semibold text-[var(--tx)]">{truckMap.get(a.truck_id) || "Truck"}</span>
                      <span className="text-caption text-[var(--tx3)]">→</span>
                      <span className="text-body font-medium text-[var(--gold)]">{teamMap.get(a.team_id) || "Team"}</span>
                    </div>
                    <button
                      type="button"
      ...
      {addModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" aria-modal="true">
          ...
          >
            <h3 className="font-heading text-h3-sm font-bold text-[var(--tx)] mb-1">Assign team to truck</h3>
            <p className="text-caption text-[var(--tx3)] mb-4">{formatDateLabel(date)}</p>
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                <label className="block text-label font-bold tracking-wider uppercase text-[var(--tx3)] mb-2">Truck</label>
                <select
                  value={formTruckId}
