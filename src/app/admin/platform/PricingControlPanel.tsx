    ...
    <div className="pt-4 space-y-4">
      ...
      {labourRow ? (
        <div className="flex items-center gap-3">
          ...
          <EditCell value={rate} onChange={(v) => updateRow(String(labourRow.id), "value", v)} type="number" className="w-20 text-[length:var(--text-base)] font-bold text-[var(--gold)]" />
        </div>
      ) : (
        <div>
      ...
      {estateSuppliesRow ? (
        <div className="flex items-center gap-3">
          ...
          <EditCell value={estateSuppliesBase} onChange={(v) => updateRow(String(estateSuppliesRow.id), "value", v)} type="number" className="w-24 text-[length:var(--text-base)] font-bold text-[var(--gold)]" />
        </div>
      ) : (
        <div>
          <p className="text-[11px] text-[var(--tx3)]">{MSG_CONFIG_MISSING}</p>
        ...
        {[
          ...
          return (
            <div key={c.key} className="bg-[var(--bg)] rounded-lg p-3">
              ...
              <EditCell value={c.val} onChange={(v) => updateRow(String(row.id), "value", v)} type="number" className="text-[length:var(--text-base)] font-bold text-[var(--tx)]" />
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          return (
            <div key={c.key} className="bg-[var(--bg)] rounded-lg p-3">
              <div className="text-[9px] font-bold uppercase tracking-wider text-[var(--tx3)] mb-1">{c.label}</div>
              <EditCell value={row.value as string} onChange={(v) => configSection.updateRow(String(row.id), "value", v)} type="number" className="text-[length:var(--text-base)] font-bold text-[var(--tx)]" />
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {floorRow ? (
          <div className="space-y-1">
            ...
            <EditCell value={floor} onChange={(v) => updateRow(String(floorRow.id), "value", v)} type="number" className="w-20 text-[length:var(--text-base)] font-bold text-[var(--grn)]" />
        ...
        {capRow ? (
          <div className="space-y-1">
            ...
            <EditCell value={cap} onChange={(v) => updateRow(String(capRow.id), "value", v)} type="number" className="w-20 text-[length:var(--text-base)] font-bold text-amber-600" />
                ...
                <div key={key} className="rounded-lg bg-[var(--bg)] border border-[var(--brd)] p-3">
                  ...
                  {row ? (
                    <EditCell
                      ...
                      onChange={(v) => updateRow(String(row.id), "value", v)}
                      type="number"
                      className="text-[length:var(--text-base)] font-bold text-[var(--gold)]"
                    />
                  ) : (
                    <div>
                      <p className="text-[10px] text-[var(--tx3)]">{MSG_CONFIG_MISSING}</p>
