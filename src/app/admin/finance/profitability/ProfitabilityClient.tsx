...
const TYPE_LABELS: Record<string, string> = {
  ...
  b2b: "B2B",
  b2b_delivery: "B2B",
  single_item_delivery: "Single Item",
...
function EditableCostCell({
  ...
  if (editing) {
    return (
      ...
      >
        ...
        <button
          onClick={cancel}
          className="text-[var(--tx3)] hover:text-red-400 p-0.5"
        >
          <X className="w-3 h-3" />
        </button>
      </div>
  ...
  return (
    ...
    >
      <span
        className={`tabular-nums ${row.hasOverride ? "underline decoration-dotted decoration-[var(--gold)]" : ""}`}
      >
...
function makeBreakdown(source: ProfitRow[]) {
  const m: Record<
    string,
    { count: number; totalRev: number; totalCost: number; totalGP: number }
  > = {};
  ...
  return Object.entries(m).map(([type, v]) => ({
    ...
    margin:
      v.totalRev > 0 ? Math.round((v.totalGP / v.totalRev) * 100 * 10) / 10 : 0,
  }));
}
...
/* ════════════ Overhead Editor ════════════ */
const STANDARD_OVERHEAD_FIELDS: {
  key: string;
  label: string;
  defaultVal: number;
  help?: string;
...
const TRUCK_FLEET = [
  ...
  // Operator-set rental rates (revised 2026-06-28). Monthly default $0
  // across rented sizes because we don't lease these.
  { key: "16ft", label: "16 ft (rental)", monthlyDefault: 0, dailyDefault: 80 },
  { key: "20ft", label: "20 ft (rental)", monthlyDefault: 0, dailyDefault: 120 },
  { key: "24ft", label: "24 ft (rental)", monthlyDefault: 0, dailyDefault: 150 },
  {
    key: "26ft",
...
function OverheadEditor({
  ...
  const [truckMonthly, setTruckMonthly] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      TRUCK_FLEET.map((t) => [
        t.key,
        config[`truck_monthly_cost_${t.key}`] ?? String(t.monthlyDefault),
      ]),
    ),
  );
  const [truckWorkingDays, setTruckWorkingDays] = useState(
    config.truck_working_days_per_month ?? "22",
  );
  const [customItems, setCustomItems] = useState<CustomOverheadItem[]>(() => {
    try {
      return JSON.parse(config.custom_overhead_items ?? "[]");
    } catch {
      return [];
    }
  });
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          {TRUCK_FLEET.map((t) => (
            <div key={t.key} className="flex items-center gap-3">
              <div className="w-24 text-[10px] font-medium text-[var(--tx)]">
                {t.label}
              </div>
              <div className="flex-1">
                <label className="block text-[9px] text-[var(--tx3)] mb-0.5">
                  Monthly lease/rental
                </label>
                <div className="relative">
                  <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[10px] text-[var(--tx3)]">
                    $
                  </span>
                  <input
                    type="number"
              ...
              <div className="w-32 text-right">
                <div className="text-[9px] text-[var(--tx3)] mb-0.5">
                  Daily rate used
                </div>
                <div className="text-[12px] font-semibold text-[var(--gold)]">
                  {formatCurrency(derivedDaily(t.key))}
                  <span className="text-[9px] text-[var(--tx3)] font-normal">
                    /day
                  </span>
                </div>
          ...
          {customItems.map((item) => (
            <div key={item.id} className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-[var(--tx3)] hover:text-red-400 transition-colors p-1"
              >
      ...
      <div className="flex items-center gap-2 pt-1">
        <input
          type="text"
          placeholder="New line item (e.g. Rent, Phone, Fuel Card...)"
          value={newLabel}
...
function useColResize(count: number, initialWidths: number[]) {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      setWidths((prev) => {
        const next = [...prev];
        next[dragging.current!.col] = Math.max(
          40,
          dragging.current!.startW + delta,
        );
        return next;
      });
    };
...
export default function ProfitabilityClient() {
  ...
  const [tableTab, setTableTab] = useState<"all" | "moves" | "deliveries">(
    "all",
  );
  const [breakdownTab, setBreakdownTab] = useState<"moves" | "deliveries">(
    "moves",
  );
  ...
  const handleCostSaved = useCallback(
    (rowId: string, field: CostField, newValue: number) => {
      setRows((prev) =>
        prev.map((r) => {
  ...
  const moveRows = useMemo(
    () => rows.filter((r) => r.jobKind === "move"),
    [rows],
  );
  const deliveryRows = useMemo(
    () => rows.filter((r) => r.jobKind === "delivery"),
    [rows],
  );
  ...
  const revenueSplit = useMemo(() => {
    ...
    return {
      residential: {
        ...res,
        margin:
          res.revenue > 0 ? Math.round((res.gp / res.revenue) * 1000) / 10 : 0,
      },
      b2b: {
        ...b2b,
        margin:
          b2b.revenue > 0 ? Math.round((b2b.gp / b2b.revenue) * 1000) / 10 : 0,
      },
      combined: {
        revenue: res.revenue + b2b.revenue,
        gp: res.gp + b2b.gp,
        count: res.count + b2b.count,
        margin:
          res.revenue + b2b.revenue > 0
            ? Math.round(
                ((res.gp + b2b.gp) / (res.revenue + b2b.revenue)) * 1000,
              ) / 10
            : 0,
      },
    };
  ...
  const tierBreakdown = useMemo(() => {
    ...
    return Object.entries(m)
      ...
      .map(([tier, v]) => ({
        ...
        margin:
          v.totalRev > 0
            ? Math.round((v.totalGP / v.totalRev) * 100 * 10) / 10
            : 0,
      }));
  }, [moveRows]);
  ...
  const marginFlagBreakdown = useMemo(() => {
    ...
    const yellow = rows.filter(
      (r) => r.grossMargin >= 25 && r.grossMargin < 35,
    );
  ...
  const neighbourhoodBreakdown = useMemo(() => {
    const m: Record<
      string,
      { count: number; totalRev: number; totalGP: number }
    > = {};
    for (const r of rows) {
      if (!r.neighbourhood) continue;
      if (!m[r.neighbourhood])
        m[r.neighbourhood] = { count: 0, totalRev: 0, totalGP: 0 };
  ...
  const filteredRows = useMemo(() => {
    ...
    if (marginFilter === "yellow")
      r = r.filter((row) => row.grossMargin >= 25 && row.grossMargin < 35);
  ...
  const exportCSV = () => {
    ...
    const csvRows = filteredRows.map((r) =>
      [
        r.move_code,
        r.jobKind === "move" ? "Move" : "Partner Job",
        ...
        <thead>
          <tr>
            {[
              ...
              <th
                key={h}
                className="text-left text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] py-2 px-2"
              >
                {h}
              </th>
            ))}
        ...
        <tbody>
          {data.map((t) => (
            <tr key={t.type} className="border-t border-[var(--brd)]/50">
              ...
              <td
                className={`py-1.5 px-2 font-semibold ${marginColor(t.margin, target)}`}
              >
                {pct(t.margin)}
              </td>
            </tr>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 border-t border-[var(--brd)]/30 pt-5">
            <StatCard
              ...
              icon={
                (summary?.avgGrossMargin ?? 0) >= target ? (
          ...
          {marginFlagBreakdown.total > 0 && (
            <div className="flex items-center justify-center gap-8 border-t border-[var(--brd)]/30 pt-4">
              ...
              <button
                type="button"
                onClick={() =>
                  setMarginFilter((f) => (f === "yellow" ? "" : "yellow"))
                }
              ...
              >
                <span className="w-2 h-2 rounded-full bg-[var(--admin-primary-fill)] shrink-0" />
                <div className="text-left">
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-[18px] font-bold font-heading text-[var(--tx)] leading-none">
                      {marginFlagBreakdown.yellow.length}
                    </span>
          ...
          {/* ─── Revenue Split ─── */}
          {(revenueSplit.b2b.count > 0 ||
            revenueSplit.residential.count > 0) && (
            <div className="grid grid-cols-3 gap-3 border-t border-[var(--brd)]/30 pt-5">
              <div className="py-3 space-y-1">
                ...
                <div className="text-[18px] font-bold text-[var(--tx)]">
                  {formatCurrency(revenueSplit.residential.revenue)}
                </div>
                <div className="text-[11px] text-[var(--tx3)]">
                  {revenueSplit.residential.count} moves ·{" "}
                  {pct(revenueSplit.residential.margin)} margin
                </div>
              </div>
              <div className="py-3 space-y-1">
                <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/60">
                ...
                <div className="text-[18px] font-bold text-[var(--tx)]">
                  {formatCurrency(revenueSplit.b2b.revenue)}
                </div>
                <div className="text-[11px] text-[var(--tx3)]">
                  {revenueSplit.b2b.count} deliveries ·{" "}
                  {pct(revenueSplit.b2b.margin)} margin
                </div>
              </div>
              <div className="py-3 space-y-1">
                <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/60">
                ...
                <div className="text-[18px] font-bold text-[var(--tx)]">
                  {formatCurrency(revenueSplit.combined.revenue)}
                </div>
                <div className="text-[11px] text-[var(--tx3)]">
                  {revenueSplit.combined.count} total ·{" "}
                  {pct(revenueSplit.combined.margin)} blended
                </div>
              </div>
            </div>
          )}
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {tierBreakdown.map((t) => (
                        <tr
                          key={t.tier}
                          className="border-t border-[var(--brd)]/50"
                        >
                          <td className="py-1.5 px-2 uppercase font-medium text-[var(--tx)]">
                            {t.tier}
                          </td>
                          <td className="py-1.5 px-2 text-[var(--tx2)]">
                            {t.moves}
          ...
          {(moveTypeBreakdown.length > 0 ||
            ...
            <Section title="Profit by Job Type">
              ...
              {(breakdownTab === "moves"
                ? [moveTypeBreakdown, "#2C3E2D"]
                : ([deliveryTypeBreakdown, "#2D6A4F"] as const)
              ).length > 0 &&
                (() => {
                  const data =
                    breakdownTab === "moves"
                  ...
                  <tbody>
                    {neighbourhoodBreakdown.slice(0, 15).map((n) => (
                      <tr
                        ...
                        className="border-t border-[var(--brd)]/50"
                      >
                        <td className="py-1.5 px-2 font-mono font-medium text-[var(--tx)]">
                          {n.area}
                        </td>
                        <td className="py-1.5 px-2 text-[var(--tx2)]">
                          {n.moves}
                        ...
                        <td
                          className={`py-1.5 px-2 font-semibold ${marginColor(n.margin, target)}`}
                        >
                          {pct(n.margin)}
                        </td>
                      </tr>
              ...
              <div className="flex items-center gap-1 bg-[var(--bg)] rounded-lg p-1">
                {(
                  [
                    ...
                    {
                      key: "deliveries",
                      label: `Partner Jobs  (${deliveryRows.length})`,
                    },
                  ] as const
                ).map(({ key, label }) => (
                  <button
                ...
                <thead>
                  <tr className="border-b border-[var(--brd)]/40">
                    {[
                      ...
                      <th
                        key="client"
                        className="relative text-left text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] py-2 px-2 whitespace-nowrap"
                      >
                        Client
                        <ColHandle col={2} />
                      </th>,
                      ...
                      <th
                        key="dc"
                        className="relative text-left text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] py-2 px-2 whitespace-nowrap cursor-pointer hover:text-[var(--tx)] select-none"
                        onClick={() => toggleSort("totalDirect")}
                      >
                        <span className="inline-flex items-center gap-0.5">
                          Direct Cost
                          <ArrowUpDown className="w-2.5 h-2.5 opacity-40" />
                  ...
                  {filteredRows.slice(0, visibleCount).map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-1.5 px-2 font-medium text-[var(--tx)] overflow-hidden text-ellipsis whitespace-nowrap">
                        ...
                        <td
                          className={`py-1.5 px-2 font-bold overflow-hidden text-ellipsis whitespace-nowrap ${marginColor(r.grossMargin, target)}`}
                        >
                          {pct(r.grossMargin)}
                        </td>
                        {(() => {
            ...
            {visibleCount < filteredRows.length && (
              <div className="text-center pt-3">
                <button
                  ...
                  className="text-[11px] font-medium text-[var(--gold)] hover:opacity-80 transition-opacity"
                >
          ...
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl">
            ...
            >
              ...
              <ChevronDown
                weight="regular"
                className={`w-4 h-4 text-[var(--tx3)] transition-transform shrink-0 ${showOverhead ? "rotate-180" : ""}`}
              />
            </button>
