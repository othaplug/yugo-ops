...
export default function GenerateInvoiceButton({ delivery }: { delivery: any }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.ok) toast(`Invoice ${data.invoice.invoice_number} created`, "💰");
  ...
  return (
    <button 
      ...
      disabled={loading}
      className="px-4 py-2 rounded-lg text-[11px] font-semibold bg-[var(--gold)] text-[#0D0D0D] hover:bg-[var(--gold2)] transition-all disabled:opacity-50"
    >
