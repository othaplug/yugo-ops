...
export default async function NewDeliveryPage() {
  ...
  const [{ data: orgs }, { data: crews }] = await Promise.all([
    db.from("organizations").select("id, name, type, email, contact_name, phone, default_pickup_address").not("name", "like", "\\_%").order("name"),
  ...
  return (
    <div className="max-w-[640px] mx-auto px-5 md:px-6 py-5 animate-fade-up">
      ...
      <NewDeliveryChoiceClient organizations={orgs || []} crews={crews || []} />
    </div>
  );
}
