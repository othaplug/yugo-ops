...
export default function CrewReportsTab({
  initialReports,
  initialDate,
  initialFrom,
  initialTo,
  ...
  const filteredReports = useMemo(() => {
    let list = reports;
    if (filterTeamId !== "all")
      list = list.filter((r) => r.team_id === filterTeamId);
  ...
  const exportFilename =
    from === to ? `eod-reports-${from}` : `eod-reports-${from}-${to}`;
          ...
          <div className="relative">
            ...
            >
              ...
              <CaretDown
                size={12}
                weight="regular"
                className={`shrink-0 transition-transform text-current ${filterOpen ? "rotate-180" : ""}`}
              />
            </button>
            {filterOpen && (
              <div className="absolute top-full right-0 mt-1.5 w-[280px] rounded-lg bg-[var(--card)] shadow-xl z-50 overflow-hidden">
                <div className="p-4 space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-[10px] font-medium text-[var(--tx3)] mb-1">
                        Single date
                      </label>
                      <input
                        type="date"
          ...
          {filteredReports.map((r) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
                <h2 className="admin-section-h2">{getCrewName(r.crews)}</h2>
                <span className="text-[12px] text-[var(--tx3)]">
                  {r.generated_at
                    ? new Date(r.generated_at).toLocaleTimeString("en-US", {
                        hour: "numeric",
                        minute: "2-digit",
                      })
                    : "-"}
              ...
              <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4">
                    <div>
                      ...
                      <div className="text-[16px] font-bold font-heading text-[var(--tx)] mt-0.5">
                        {String(r.summary.jobsCompleted ?? 0)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50">
                      ...
                      <div
                        className={`text-[16px] font-bold font-heading mt-0.5 ${(r.jobs?.filter((j) => j.hasDamage).length ?? 0) > 0 ? "text-[var(--org)]" : "text-[var(--tx)]"}`}
                      >
                        {r.jobs?.filter((j) => j.hasDamage).length ?? 0}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50">
                      ...
                      <div className="text-[16px] font-bold font-heading text-[var(--tx)] mt-0.5">
                        {Math.floor((Number(r.summary.totalJobTime) || 0) / 60)}
                        h {(Number(r.summary.totalJobTime) || 0) % 60}m
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50">
                      ...
                      <div className="text-[16px] font-bold font-heading text-[var(--tx)] mt-0.5">
                        {String(r.summary.clientSignOffs ?? 0)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50">
                      ...
                      <div className="text-[16px] font-bold font-heading text-[var(--tx)] mt-0.5">
                        $
                        {((Number(r.summary.expensesTotal) || 0) / 100).toFixed(
                          2,
                        )}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="text-[16px] font-bold font-heading text-[var(--tx)] mt-0.5">
                        {typeof r.summary.averageSatisfaction === "number" ||
                        typeof r.summary.averageSatisfaction === "string"
                        ...
                        >
                          <div className="flex justify-between items-start gap-2">
                            <div className="min-w-0">
                              <div className="font-heading font-semibold text-[var(--tx)] text-[13px] truncate">
                                {j.displayId ?? "Job"}
                              </div>
                              <div className="text-[12px] text-[var(--tx2)] truncate mt-0.5">
                                {j.clientName}
                              </div>
                              <div className="text-[11px] text-[var(--tx3)] mt-1 uppercase">
                                {j.type}
                              </div>
                            </div>
                ...
                {r.crew_note && (
                  <div className="border-t border-[var(--brd)]/30 pt-5">
                    ...
                    <p className="text-[13px] text-[var(--tx2)] whitespace-pre-wrap font-heading">
                      {r.crew_note}
                    </p>
                  </div>
                )}
                {r.readiness &&
                !r.readiness.passed &&
                r.readiness.flaggedItems?.length ? (
                  <div className="border-t border-[var(--brd)]/30 pt-5">
                    <div className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--org)] mb-1">
                      Readiness flagged
                    </div>
                    <p className="text-[12px] text-[var(--tx2)]">
                      {r.readiness.flaggedItems.join(", ")}
                    </p>
                  </div>
                ) : null}
              </div>
