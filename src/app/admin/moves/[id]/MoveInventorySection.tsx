...
export default function MoveInventorySection({
  moveId,
  moveStatus,
  userRole = "viewer",
  itemWeights = [],
  ...
  const [approveExtraModal, setApproveExtraModal] = useState<{
    itemId: string;
  } | null>(null);
  ...
  const bulkSelectedCount = parsedBulkItems.filter((_, i) =>
    bulkSelected.has(i),
  ).length;
  useEffect(() => {
  ...
  const handleAdd = () => {
    ...
    fetch(`/api/admin/moves/${moveId}/inventory`, {
      ...
      body: JSON.stringify({
        room: newRoom,
        item_name: newItemQty > 1 ? `${name} x${newItemQty}` : name,
      }),
    })
      .then((r) => r.json())
      .then((data) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const r = await fetch(
        `/api/admin/moves/${moveId}/inventory/${deleteConfirm.id}`,
        { method: "DELETE" },
      );
      const data = await r.json();
      if (!r.ok || data.error) {
  ...
  const expandItemRow = (
    ...
    return rows.map((r) => ({
      label: uppercase(r.label || item.item_name),
      qty: r.qty,
    }));
  };
    ...
    <div className="bg-[var(--card)] border border-[var(--brd)]/50 rounded-xl p-4">
            ...
            {canEditInventory && editingBoxCount ? (
              <>
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") saveBoxCount();
                    if (e.key === "Escape") {
                      setEditingBoxCount(false);
                      setBoxCountInput(String(boxCount));
                    }
                  }}
                ...
                <button
                  type="button"
                  onClick={saveBoxCount}
                  disabled={savingBoxCount}
                  className="text-[10px] font-semibold text-[var(--grn)] hover:opacity-80 disabled:opacity-50"
                >
                  Save
                </button>
                <button
                  ...
                  className="text-[10px] text-[var(--tx3)]"
                >
                  Cancel
                </button>
              </>
              ...
              <button
                ...
                className={`flex items-center gap-1 px-2 py-0.5 rounded-md border text-[11px] font-semibold tabular-nums ${
                  canEditInventory
                    ? "border-[var(--brd)] hover:border-[var(--brd)]/80 cursor-pointer"
              ...
              >
                {boxCount}
                {canEditInventory && (
                  <Pencil className="w-[10px] h-[10px] text-[var(--tx3)]" />
                )}
              </button>
            )}
          </div>
        ...
        <>
          {canEditInventory && itemWeights.length > 0 && (
            <div className="mb-4 pb-4 border-b border-[var(--brd)]/40">
              <p className="text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)]/88 mb-2">
          ...
          {rooms.length === 0 && !adding ? (
            <p className="text-[11px] text-[var(--tx3)] mb-4">
              No items yet. Add room-by-room items for the client portal.
            </p>
          ) : (
            <div className="space-y-2 mb-4">
              {rooms.map((room) => {
                ...
                return (
                  <div
                    key={room}
                    className="rounded-lg overflow-hidden border border-[var(--brd)]/40 transition-colors hover:border-[var(--brd)]/60"
                  >
                    <button
                      type="button"
                    ...
                    >
                      <span className="text-[9px] font-bold tracking-wider uppercase text-[var(--gold)] group-hover:text-[var(--gold2)] transition-colors">
                        {room}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <span className="text-[9px] font-medium text-[var(--tx3)]">
                          {itemCount} item{itemCount !== 1 ? "s" : ""}
                        </span>
                        <ChevronDown
                          className={`w-[14px] h-[14px] text-[var(--tx3)] transition-transform duration-200 ease-out ${expanded ? "rotate-0" : "-rotate-90"}`}
                        />
                      </span>
                    </button>
                    <div
                        ...
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="border-b border-[var(--brd)]/40">
                              <th className="text-[9px] font-semibold uppercase text-[var(--tx3)] px-3 py-2 w-[1%] whitespace-nowrap">
                                Item
                              </th>
                              <th className="text-[9px] font-semibold uppercase text-[var(--tx3)] px-3 py-2 text-right w-14">
                                Qty
                              </th>
                              {canEditInventory && (
                          ...
                          <tbody>
                            {byRoom[room].flatMap((item) => {
                              ...
                              return rows.map((r, ri) => (
                                <tr
                                  key={`${item.id}-${ri}`}
                                  className="border-b border-[var(--brd)]/30 last:border-0 group hover:bg-[var(--bg)]/20 transition-colors"
                                >
                                  ...
                                  <td className="px-3 py-2 text-[12px] text-[var(--tx2)] text-right tabular-nums">
                                    {r.qty}
                                  </td>
                                  {canEditInventory && (
                                          ...
                                          {editingId === item.id ? (
                                            <>
                                              <button
                                                ...
                                                className="px-1.5 py-0.5 rounded text-[10px] font-semibold text-[var(--grn)] hover:bg-[var(--grdim)]"
                                              >
                                                Save
                                              </button>
                                              <button
                                                ...
                                                className="px-1.5 py-0.5 rounded text-[10px] text-[var(--tx3)]"
                                              >
                                                Cancel
                                              </button>
                                            </>
                                            ...
                                            <>
                                              <button
                                                ...
                                                className="p-1 rounded text-[var(--tx3)] hover:bg-[var(--gdim)] hover:text-[var(--gold)]"
                                                aria-label="Edit"
                                              >
                                              ...
                                              <button
                                                ...
                                                className="p-1 rounded text-[var(--tx3)] hover:bg-[var(--rdim)] hover:text-[var(--red)]"
                                                aria-label="Delete"
                                              >
          ...
          {!extraLoading && extraItems.length > 0 && (
            <div className="mb-4 pt-4 border-t border-[var(--brd)]/40">
              <h4 className="text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-2">
                Extra items
              </h4>
                      ...
                      {pending && (
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            type="button"
                            onClick={() => {
                              setApproveExtraModal({ itemId: e.id });
                              setApproveExtraFeeDollars("");
                            }}
          ...
          <div className="space-y-3 pt-1 border-t border-[var(--brd)]/40">
            {!canEditInventory ? (
              <p className="text-[11px] text-[var(--tx3)]">
                Inventory locked for completed moves. Only owners can add or
                edit items.
              </p>
            ) : (
              <>
                ...
                {bulkMode ? (
                  <div className="flex flex-col gap-2">
                    <div>
                      <label className="block text-[9px] font-medium tracking-wider uppercase text-[var(--tx3)] mb-0.5">
                        Room
                      </label>
                      <select
                        value={newRoom}
                    ...
                    <div>
                      <label className="block text-[9px] font-medium tracking-wider uppercase text-[var(--tx3)] mb-0.5">
                        Items (comma or newline, e.g. Table x1, Couch x2)
                      </label>
                      <textarea
                        value={bulkText}
                      ...
                      <div className="rounded-md border border-[var(--brd)] bg-[var(--bg)]/50 overflow-hidden">
                        ...
                        <ul className="divide-y divide-[var(--brd)]/30 max-h-[200px] overflow-y-auto">
                          {parsedBulkItems.map((item, i) => (
                            <li
                              key={i}
                              className="flex items-center gap-2 px-2 py-2 hover:bg-[var(--bg)]/30 transition-colors"
                            >
                              <input
                                type="checkbox"
                                id={`bulk-${i}`}
                              ...
                              <label
                                htmlFor={`bulk-${i}`}
                                className="text-[12px] font-medium text-[var(--tx)] cursor-pointer flex-1"
                              >
                                {item}
                              </label>
                            </li>
                    ...
                    <button
                      ...
                      className="admin-btn admin-btn-sm admin-btn-primary self-start"
                    >
        ...
        >
          <div className="p-5 space-y-4">
            <p className="text-[12px] text-[var(--tx2)]">
              Are you sure you want to remove &quot;{deleteConfirm.item_name}
      ...
      {approveExtraModal && (
        <ModalDialogFrame
          ...
          panelClassName="bg-[var(--yu3-bg-surface)] text-[var(--yu3-ink)] border border-[var(--yu3-line)] shadow-[var(--yu3-shadow-lg)] rounded-[var(--yu3-r-xl)] p-5 w-full max-w-sm modal-card"
