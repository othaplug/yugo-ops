...
export default function MoveDocumentsSection({ moveId }: { moveId: string }) {
  ...
  const handleDeleteDoc = (item: DocItem) => {
    ...
    if (item.source === "move") {
      fetch(`/api/admin/moves/${moveId}/documents/${item.id}`, { method: "DELETE" })
        .then((r) => r.json())
        .then((data) => {
          if (data.error) throw new Error(data.error);
          fetchData();
      ...
      fetch(`/api/admin/client-documents/${item.id}`, { method: "DELETE" })
        .then((r) => r.json())
        .then((data) => {
          if (data.error) throw new Error(data.error);
          fetchData();
                ...
                {linkedInvoices.map((inv) => (
                  <li key={inv.id} className="flex items-center justify-between gap-2 text-[11px]">
                    <span className="text-[var(--tx)]">
                      {inv.invoice_number} — {inv.client_name} (${Number(inv.amount).toLocaleString()})
                    </span>
                    <button
                      type="button"
