          ...
          <div className="md:hidden border-b border-[var(--brd)] bg-[var(--bg)]/80 px-3 py-4 space-y-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-full text-[11px] bg-[var(--card)] border border-[var(--brd)] rounded-lg px-3 py-2 text-[var(--tx)] focus:border-[var(--gold)] outline-none min-h-[40px] touch-manipulation" />
              </div>
              <div>
                <label className="block text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-1">End date</label>
                <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-full text-[11px] bg-[var(--card)] border border-[var(--brd)] rounded-lg px-3 py-2 text-[var(--tx)] focus:border-[var(--gold)] outline-none min-h-[40px] touch-manipulation" />
              </div>
              {hasActiveFilters && (
                <button type="button" onClick={clearFilters} className="text-[11px] font-semibold text-[var(--tx3)] hover:text-[var(--gold)]">Clear all filters</button>
              )}
            </div>
          </div>
        )}
              ...
              <Link href={getMoveDetailPath(m)} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-2.5 flex-1 min-w-0 sm:flex-nowrap">
                <div className="flex items-start sm:items-center gap-2.5 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[11px] font-semibold text-[var(--tx)] break-words line-clamp-2">{m.client_name || "—"}</div>
