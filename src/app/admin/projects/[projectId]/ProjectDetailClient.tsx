...
function EditProjectModal({
  ...
  const [estimatedBudget, setEstimatedBudget] = useState(
    data.estimated_budget != null ? String(data.estimated_budget) : "",
  );
  const [projectMgmtFee, setProjectMgmtFee] = useState(
    data.project_mgmt_fee != null ? String(data.project_mgmt_fee) : "",
  );
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <AddressAutocomplete
            value={siteAddress}
            onChange={(r) => setSiteAddress(r.fullAddress)}
...
export default function ProjectDetailClient({
  ...
  const [activeTab, setActiveTab] = useState(
    fromDesigners ? "Vendor Tracker" : "Overview",
  );
  ...
  if (loading) {
    return (
      <div className="px-6 py-8">
        <div className="animate-pulse h-8 w-48 bg-[var(--brd)] rounded mb-4" />
        <div className="animate-pulse h-64 bg-[var(--brd)] rounded" />
      </div>
    );
  }
  ...
  const activeDeliveryTotal = data.deliveries
    .filter((d) => d.status !== "cancelled")
    .reduce((s, d) => s + (d.total_price || 0), 0);
  const projectEstimate =
    activeDeliveryTotal > 0
      ? activeDeliveryTotal + (data.project_mgmt_fee || 0)
      : (data.estimated_budget || 0) + (data.project_mgmt_fee || 0);
  const estimateFromDeliveries = activeDeliveryTotal > 0;
  const manualItemsExist = data.inventory.some(
    (i) => i.handled_by && i.handled_by !== "yugo",
  );
  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      ...
      <div className="mt-4 mb-6">
        <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-[var(--tx3)]/82 mb-2">
          B2B Operations · Project
        </p>
        <div className="flex flex-wrap items-center gap-3 mb-2">
          ...
          <span
            className={`dt-badge tracking-[0.04em] ${STATUS_COLORS[data.status] || "text-[var(--tx3)]"}`}
          >
        ...
        <div className="flex flex-wrap gap-4 text-[12px] text-[var(--tx3)]">
          ...
          <span>
            Estimate:{" "}
            <span className="text-[var(--tx)] font-medium">
              {formatCurrency(projectEstimate)}
              {estimateFromDeliveries ? "" : " (initial)"}
            </span>
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mt-3">
          <div className="flex flex-wrap gap-2">
            {data.status === "draft" && (
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 transition-colors disabled:opacity-40"
              >
                <Send size={12} weight="regular" className="inline mr-1" /> Send
                Proposal
              </button>
            )}
            {data.status === "proposed" && (
              <button
                ...
                disabled={statusUpdating}
                className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 transition-colors disabled:opacity-40"
              >
                <CheckCircle2
                  size={12}
                  weight="regular"
            ...
            {data.status === "active" && (
              <>
                <button
                  ...
                  disabled={statusUpdating}
                  className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 transition-colors disabled:opacity-40"
                >
                  <CheckCircle2
                    size={12}
                    weight="regular"
                ...
                <button
                  ...
                  disabled={statusUpdating}
                  className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 transition-colors disabled:opacity-40"
                >
                  Hold
                </button>
              </>
            ...
            {data.status === "completed" && (
              <button
                ...
                disabled={statusUpdating}
                className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-purple-500/10 text-purple-500 hover:bg-purple-500/20 transition-colors disabled:opacity-40"
              >
                <FileText size={12} weight="regular" className="inline mr-1" />{" "}
                Mark Invoiced
              </button>
            )}
          </div>
          <button
            ...
            className={`${ADMIN_TOOLBAR_SECONDARY_ACTION_CLASS} shrink-0`}
          >
            <Pencil
              size={12}
              weight="regular"
          ...
          <button
            ...
            className={`${ADMIN_TOOLBAR_DESTRUCTIVE_ACTION_CLASS} shrink-0`}
          >
            <Trash2
              size={12}
              weight="regular"
        ...
        <div className="mb-4 flex items-start gap-3 p-4 rounded-xl border border-[var(--gold)]/30 bg-[var(--gold)]/5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-semibold text-[var(--tx)]">
              ...
              items are being tracked manually
            </p>
            <p className="text-[11px] text-[var(--tx3)] mt-0.5">
              Want Yugo to handle pickup and delivery with full tracking,
      ...
      <div className="tab-content">
        {activeTab === "Overview" && (
          <OverviewTab
            ...
            estimateFromDeliveries={estimateFromDeliveries}
          />
        )}
        {activeTab === "Phases" && (
          <PhasesTab
            ...
            setShowAddPhase={setShowAddPhase}
          />
        )}
        {activeTab === "Inventory" && (
          <InventoryTab
            ...
            showReceiveItem={showReceiveItem}
            setShowReceiveItem={setShowReceiveItem}
          />
        )}
        {activeTab === "Vendor Tracker" && (
          <VendorTrackerTab
            data={data}
            projectId={projectId}
            onRefresh={loadProject}
        ...
        {activeTab === "Invoice" && (
          <InvoiceTab data={data} projectId={projectId} />
        )}
      </div>
      ...
      >
        <div className="p-5 space-y-4">
          <p className="text-[12px] text-[var(--tx2)]">
            ...
            ? Deliveries will be unlinked. This action cannot be undone.
          </p>
...
function OverviewTab({
  ...
  estimateFromDeliveries: boolean;
}) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="divide-y divide-[var(--brd)]/50 pt-0 pb-6 lg:pb-0 lg:pr-6">
        ...
        <section className="py-5">
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-5 space-y-2">
            ...
            />
            {data.end_client_name && (
              <InfoRow label="End Client" value={data.end_client_name} />
            )}
            {data.end_client_contact && (
              <InfoRow label="Contact" value={data.end_client_contact} />
            )}
            {data.site_address && (
              <InfoRow label="Site" value={data.site_address} />
            )}
            <InfoRow
              label="Start"
              value={
            ...
            <InfoRow
              ...
              value={
                data.target_end_date
                  ? formatPlatformDisplay(
                      new Date(data.target_end_date + "T00:00:00"),
                      { month: "long", day: "numeric" },
                    )
                  : "TBD"
              }
      ...
      <div className="divide-y divide-[var(--brd)]/50 border-t lg:border-t-0 lg:border-l border-[var(--brd)]/50 pt-5 lg:pt-0 pl-0 lg:pl-6">
        ...
        <section className="py-5 first:pt-0">
          ...
          <div className="admin-page-hero-stat text-[var(--tx)]">
            {formatCurrency(projectEstimate)}
          </div>
          ...
          {data.project_mgmt_fee ? (
            <div className="mt-3 pt-3 border-t border-[var(--brd)] flex justify-between text-[11px]">
              ...
              <span className="text-[var(--tx)]">
                {formatCurrency(data.project_mgmt_fee)}
              </span>
            </div>
          ) : null}
        </section>
        ...
        <section className="py-5">
          <div className="grid grid-cols-2 gap-3">
            ...
            <StatCard
              ...
              sub={`${data.deliveries.filter((d) => d.status === "delivered" || d.status === "completed").length} completed`}
            />
          </div>
        </section>
        ...
        <section className="py-5 last:pb-0">
            ...
            <div key={e.id} className="flex gap-3 py-2 text-[12px]">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-[var(--tx3)]">
                  {formatPlatformDisplay(e.created_at, {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                    minute: "2-digit",
          ...
          {data.timeline.length === 0 && (
            <div className="text-[12px] text-[var(--tx3)]">No activity yet</div>
          )}
        </section>
      </div>
    </div>
...
function PhaseItemRow({
  ...
  const [condTimeout, setCondTimeout] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
  const [storTimeout, setStorTimeout] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const handleCondition = (val: string) => {
    ...
    setCondTimeout(
      setTimeout(() => patchItem({ condition_on_receipt: val || null }), 800),
    );
  };
  ...
  const handleStorage = (val: string) => {
    ...
    setStorTimeout(
      setTimeout(() => patchItem({ storage_location: val || null }), 800),
    );
  };

  return (
    <tr className="border-b border-[var(--brd)]/50 hover:bg-[var(--gdim)]/10">
      <td className="px-2 py-2 text-[12px] font-medium text-[var(--tx)]">
        {item.item_name}
      </td>
...
function PhasesTab({
  ...
  setShowAddPhase: (v: boolean) => void;
}) {
  ...
  const [expandedPhase, setExpandedPhase] = useState<string | null>(
    data.phases.find((p) => p.status === "active")?.id ||
      data.phases[0]?.id ||
      null,
  );
  ...
  const addPhase = async () => {
    ...
    await fetch(`/api/admin/projects/${projectId}/phases`, {
      ...
      body: JSON.stringify({
        ...
        scheduled_date: newPhaseDate || null,
        address: newPhaseAddress || null,
      }),
    });
    setNewPhaseName("");
    setNewPhaseDate("");
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border transition-colors shrink-0 ${
                  expandedPhase === phase.id
                    ? "border-[var(--gold)] bg-[var(--gdim)]/30"
                    : "border-[var(--brd)] hover:border-[var(--gold)]/50"
                }`}
              >
                <Icon size={14} className={cfg.text} />
                <div className="text-left">
                  <div className="text-[11px] font-semibold text-[var(--tx)] whitespace-nowrap">
                    {phase.phase_name}
                  </div>
                  <div className="text-[9px] text-[var(--tx3)] uppercase">
                    {phase.status}
                    {phase.scheduled_date
                      ? ` · ${formatMoveDate(phase.scheduled_date)}`
                      : ""}
                    {phase.address ? ` · ${phase.address.split(",")[0]}` : ""}
                  </div>
                </div>
                {i < data.phases.length - 1 && (
                  <span className="text-[var(--tx3)] ml-1">→</span>
                )}
              </button>
            );
          })}
      ...
      {expandedPhase && (
        <section className="py-5">
          {(() => {
            ...
            const phaseItems = data.inventory.filter(
              (i) => i.phase_id === phase.id,
            );
            const phaseDeliveries = data.deliveries.filter(
              (d) => d.phase_id === phase.id,
            );
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {phase.status === "pending" && (
                      <button
                        ...
                        className="px-3 py-1 rounded-lg text-[10px] font-semibold bg-amber-500/10 text-amber-500 hover:bg-amber-500/20"
                      >
                        Start Phase
                      </button>
                    )}
                    {phase.status === "active" && (
                      <button
                        ...
                        className="px-3 py-1 rounded-lg text-[10px] font-semibold bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20"
                      >
                        Complete Phase
                      </button>
                    )}
                      ...
                      <thead>
                        <tr className="border-b border-[var(--brd)]">
                          {[
                            ...
                            <th
                              key={h}
                              className="px-2 py-2 text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)]"
                            >
                              {h}
                            </th>
                          ))}
                  ...
                  <div>
                    ...
                    {phaseDeliveries.map((d) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-semibold uppercase text-[var(--tx3)]">
                          {d.status}
                        </span>
                      </Link>
                    ))}
        ...
        {showAddPhase ? (
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4 space-y-3">
            <input
              ...
              placeholder="Phase name (e.g. Receiving, Delivery, Installation)…"
              className={fieldInput}
              autoFocus
            />
            <input
          ...
          <button
            ...
            className="admin-btn admin-btn-secondary border-dashed"
          >
...
function InventoryTab({
  ...
  showReceiveItem: string | null;
  setShowReceiveItem: (v: string | null) => void;
}) {
  ...
  const [newItemDeliveryMethod, setNewItemDeliveryMethod] =
    useState("yugo_pickup");
  ...
  const [updateVendorItemId, setUpdateVendorItemId] = useState<string | null>(
    null,
  );
  ...
  const canReceive = (status: string) =>
    !PROJECT_ITEM_RECEIVED_STATUSES.includes(
      status as (typeof PROJECT_ITEM_RECEIVED_STATUSES)[number],
    ) &&
    !isComplete(status) &&
  ...
  return (
    <div className="divide-y divide-[var(--brd)]/50">
        ...
        <section className="py-5">
          <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-4 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              />
              <input
                value={newItemContactName}
                onChange={(e) => setNewItemContactName(e.target.value)}
              ...
              />
              <input
                value={newItemOrderNumber}
                onChange={(e) => setNewItemOrderNumber(e.target.value)}
              ...
              />
              <input
                value={newItemPickupWindow}
                onChange={(e) => setNewItemPickupWindow(e.target.value)}
              ...
              <select
                value={newItemDeliveryMethod}
                onChange={(e) => setNewItemDeliveryMethod(e.target.value)}
                className={fieldInput}
              >
              ...
              <input
                value={newItemCarrier}
                onChange={(e) => setNewItemCarrier(e.target.value)}
                placeholder="Carrier (e.g., FedEx, DHL)"
                className={fieldInput}
              />
              <input
                value={newItemTrackingNum}
                onChange={(e) => setNewItemTrackingNum(e.target.value)}
                placeholder="Tracking number"
                className={fieldInput}
              />
              <input
                type="date"
                value={newItemExpectedDate}
                onChange={(e) => setNewItemExpectedDate(e.target.value)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <label className="flex items-center gap-2 text-[12px] text-[var(--tx)]">
                <input
                  type="checkbox"
                  checked={newItemCrating}
                  onChange={(e) => setNewItemCrating(e.target.checked)}
                />
                Crating required
              </label>
              <label className="flex items-center gap-2 text-[12px] text-[var(--tx)]">
                <input
                  type="checkbox"
                  checked={newItemAssembly}
                  onChange={(e) => setNewItemAssembly(e.target.checked)}
                />
                Assembly required
              </label>
            </div>
            <textarea
              ...
              placeholder="Special handling / status notes"
              rows={3}
              className={fieldInput}
            />
            <div className="flex gap-2">
          ...
          return (
            <section className="py-5">
              <div className="bg-[var(--card)] border border-[var(--gold)]/30 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] text-[var(--tx3)] mb-1 block">
                      Status
                    </label>
                    <select
                      value={updateStatus}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="e.g., Delayed, customs hold, expected +5 days"
                    rows={2}
                    className={fieldInput}
                  />
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={updateVendorItem}
                    className="admin-btn admin-btn-primary"
                  >
                    Save
                  </button>
                  <button
                    onClick={() => setUpdateVendorItemId(null)}
                    className="admin-btn admin-btn-secondary"
                  >
                    Cancel
                  </button>
              ...
              <div className="bg-[var(--card)] border border-[var(--gold)]/30 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      value={receiveStorage}
                      onChange={(e) => setReceiveStorage(e.target.value)}
                      placeholder="e.g., Bay 3, Shelf 2"
                      className={fieldInput}
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={receiveItem}
                    className="px-4 py-2 rounded-lg text-[11px] font-semibold bg-emerald-500 text-white hover:bg-emerald-600"
                  >
                    Mark Received
                  </button>
                  <button
                    onClick={() => setShowReceiveItem(null)}
                    className="px-4 py-2 rounded-lg text-[11px] font-semibold border border-[var(--brd)] text-[var(--tx3)]"
                  >
                    Cancel
                  </button>
                </div>
      ...
      <section className="py-5 last:pb-0">
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-[var(--tx3)] text-[13px]">
          ...
          <div className="overflow-x-auto">
              ...
              <tbody>
                {filtered.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="border-b border-[var(--brd)]/50 hover:bg-[var(--gdim)]/20"
                    >
                      ...
                      <td className="px-2 py-2.5 text-[11px] text-[var(--tx3)]">
                        {item.vendor_name || item.vendor || "-"}
                      </td>
                      <td className="px-2 py-2.5">
                        <span
                          className={`dt-badge tracking-[0.04em] ${badge.text}`}
                        >
                          {badge.label}
                        </span>
                      </td>
                      ...
                      <td className="px-2 py-2.5 text-[11px]">
                        {handler === "yugo" ? (
                          ...
                          >
                            {item.vendor_tracking_number.slice(0, 12)}
                            {item.vendor_tracking_number.length > 12 ? "…" : ""}
                          </a>
                        ) : (
                      ...
                      <td className="px-2 py-2.5 text-[11px] text-[var(--tx3)]">
                        {item.received_date
                          ? formatMoveDate(item.received_date)
                          : item.expected_delivery_date
                            ? `ETA ${formatMoveDate(item.expected_delivery_date)}`
                            : "-"}
                      </td>
                      <td className="px-2 py-2.5">
                        ...
                        {handler !== "yugo" && !isComplete(status) && (
                          <>
                            ...
                            {canReceive(status) && (
                              <button
                                onClick={() => setShowReceiveItem(item.id)}
                                className="text-[10px] font-semibold text-[var(--gold)] hover:underline"
                              >
                                Receive
                              </button>
            ...
            {(() => {
              const nonYugoCount = filtered.filter(
                (i) => i.handled_by && i.handled_by !== "yugo",
              ).length;
                ...
                <div className="mt-6 rounded-xl border border-[var(--gold)]/20 bg-[var(--gold)]/5 p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="text-[13px] font-semibold text-[var(--tx)]">
                        {nonYugoCount} item{nonYugoCount > 1 ? "s" : ""} tracked
                        manually this month
                      </div>
                      <p className="text-[11px] text-[var(--tx3)] mt-1">
                        Want guaranteed white-glove handling with real-time
                        tracking, photo documentation, and proof of delivery?
                      </p>
                      <Link
                        href={`/admin/deliveries/new?choice=single&org=${data.partner_id}&projectId=${projectId}`}
                        className="admin-btn admin-btn-primary mt-3 inline-flex"
                      >
                        <Truck size={13} weight="regular" /> Schedule these with
                        Yugo
                      </Link>
                    </div>
                  </div>
...
function DeliveriesTab({
  ...
  const updateDeliveryStatus = async (deliveryId: string, status: string) => {
    ...
    if (res.ok) {
      onRefresh();
      toast("Delivery status updated", "check");
  ...
  const readyItems = data.inventory.filter((i) => {
    ...
    return (
      st === "ready_for_pickup" || PROJECT_ITEM_WAREHOUSE_STATUSES.includes(st)
    );
  });
  const pickupGroups = Array.from(
    new Set(readyItems.map((i: any) => i.vendor_pickup_address || "")),
    ...
    <div className="divide-y divide-[var(--brd)]/50">
      <section className="py-5 first:pt-0">
        <div className="flex justify-between items-center">
          <div className="text-[12px] text-[var(--tx3)]">
            {data.deliveries.length} deliver
            {data.deliveries.length !== 1 ? "ies" : "y"} linked
          </div>
          <div className="flex gap-2">
            {pickupGroups.length > 0 &&
              pickupGroups.map((addr) => {
                const count = readyItems.filter(
                  (i: any) => i.vendor_pickup_address === addr,
                ).length;
                ...
                return (
                  ...
                  >
                    <Truck size={12} /> {count} item{count !== 1 ? "s" : ""}{" "}
                    from {short}
                  </Link>
                );
              })}
      ...
      <section className="py-5 last:pb-0">
        {data.deliveries.length === 0 ? (
          <div className="text-center py-12 text-[var(--tx3)] text-[13px]">
            No deliveries linked to this project
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-[var(--brd)]">
                  {["DLV #", "Phase", "Date", "Items", "Price", "Status"].map(
                    (h) => (
                      <th
                        key={h}
                        className="px-3 py-2 text-[9px] font-bold tracking-wider uppercase text-[var(--tx3)]"
                      >
                        {h}
                      </th>
                    ),
                ...
                {data.deliveries.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-[11px] text-[var(--tx3)]">
                        {phase?.phase_name || "-"}
                      </td>
                      <td className="px-3 py-2.5 text-[11px] text-[var(--tx3)]">
                        {d.scheduled_date
                      ...
                      <td className="px-3 py-2.5 text-[12px] font-medium text-[var(--tx)]">
...
function InvoiceTab({
  ...
  const deliveryTotal = activeDeliveries.reduce(
    (s, d) => s + (d.total_price || 0),
    0,
  );
  const effectiveDeliveryTotal =
    deliveryTotal > 0 ? deliveryTotal : data.estimated_budget || 0;
  ...
  return (
    <div className="divide-y divide-[var(--brd)]/50">
      <section className="py-5 first:pt-0">
        <div className="bg-[var(--card)] border border-[var(--brd)] rounded-xl p-5">
          <div className="text-[10px] font-bold tracking-wider uppercase text-[var(--tx3)] mb-4">
            Project Invoice
          </div>
          ...
          <table className="w-full text-left mb-4">
            ...
            <tbody>
              {activeDeliveries.length > 0 ? (
                activeDeliveries.map((d) => {
                  ...
                  return (
                    <tr key={d.id} className="border-b border-[var(--brd)]/50">
                      <td className="px-2 py-2.5 text-[12px] text-[var(--gold)] font-medium">
                        {d.delivery_number}
                      </td>
                      <td className="px-2 py-2.5 text-[12px] text-[var(--tx)]">
                        {phase ? `${phase.phase_name} delivery` : "Delivery"}
                        {!isCompleted && (
                          <span className="ml-1.5 text-[9px] font-semibold uppercase text-[var(--tx3)] opacity-60">
                            ({d.status})
                          </span>
                        )}
                      </td>
                      <td className="px-2 py-2.5 text-[12px] font-medium text-[var(--tx)]">
                        {d.total_price ? formatCurrency(d.total_price) : "-"}
                      </td>
                    </tr>
                  );
                ...
                <tr className="border-b border-[var(--brd)]/50">
                  <td className="px-2 py-2.5 text-[12px] text-[var(--tx3)]">
                    -
                  </td>
                  <td className="px-2 py-2.5 text-[12px] text-[var(--tx)]">
                  ...
                  <td className="px-2 py-2.5 text-[12px] font-medium text-[var(--tx)]">
                    {formatCurrency(data.estimated_budget)}
                  </td>
                </tr>
              ) : null}
              {mgmtFee > 0 && (
            ...
            <tfoot>
              <tr>
                <td
                  className="px-2 py-2 text-[12px] text-[var(--tx3)]"
                  colSpan={2}
                >
                  Subtotal
                </td>
              ...
              <tr className="border-t-2 border-[var(--brd)]">
                <td
                  className="px-2 py-3 text-[13px] font-bold text-[var(--tx)]"
                  colSpan={2}
                >
                  TOTAL (incl. HST)
                </td>
                <td className="px-2 py-3 text-[13px] font-bold text-[var(--gold)]">
          ...
          {activeDeliveries.length === 0 && !data.estimated_budget && (
            <div className="text-center py-8 text-[var(--tx3)] text-[12px]">
              No deliveries linked to this project yet
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-5 last:pb-0">
        ...
        {invoiceError && (
          <p className="text-[11px] text-[var(--red)]">{invoiceError}</p>
        )}
      </section>
    </div>
  );
...
const VT_STATUS_GROUPS = [
  ...
  {
    key: "ordered",
    label: "Ordered / In Production",
    color: "text-sky-400",
    statuses: ["spec_selected", "ordered", "in_production"],
  },
  {
    key: "in_transit",
    label: "In Transit",
  ...
  },
  {
    ...
    statuses: VT_RECEIVED,
  },
  {
    key: "delivered",
    label: "Delivered & Installed",
    color: "text-purple-500",
    statuses: ["delivered", "installed"],
  },
  {
    key: "issues",
    label: "Issues",
...
function VtItemCard({
  ...
  const [trackTimeout, setTrackTimeout] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
  const st = getProjectItemStatus(item);
  const cfg = getProjectItemStatusUi(st);
  const isYugo =
    item.handled_by === "yugo" || item.vendor_delivery_method === "yugo_pickup";
  const isCarrier =
    item.handled_by === "other_carrier" ||
    item.vendor_delivery_method === "shipped_carrier";
  const isVendorDirect =
    item.handled_by === "vendor_direct" ||
    item.vendor_delivery_method === "vendor_direct";

  // Find linked Yugo delivery for this item (by phase match or items array)
  const linkedDelivery = isYugo
    ? deliveries.find((d) => {
        if (Array.isArray(d.items)) {
          return d.items.some(
            (di: any) =>
              di?.id === item.id || di?.item_id === item.id || di === item.id,
          );
        }
        return false;
      }) ||
      (item.phase_id
        ? deliveries.find(
            (d) => d.phase_id === item.phase_id && d.status !== "cancelled",
          )
        : null)
    : null;
  ...
  const handleTrackingNum = (val: string) => {
    setTrackingNum(val);
    saveTracking(val, carrier);
  };
  const handleCarrier = (val: string) => {
    setCarrier(val);
    saveTracking(trackingNum, val);
  };
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      ...
      if (
        ...
        (e.target as HTMLElement)?.closest?.("[data-status-picker]")
      )
        return;
  ...
  return (
    <div className="py-2.5 space-y-2">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {isCarrier && (
              <span className="dt-badge tracking-[0.04em] text-purple-400">
                CARRIER
              </span>
            )}
            {isVendorDirect && (
              <span className="dt-badge tracking-[0.04em] text-sky-400">
                VENDOR SHIP
              </span>
            )}
          </div>
          {item.room_destination && (
          ...
          {showStatusPicker &&
            ...
            createPortal(
              <Yu3PortaledTokenRoot
                data-status-picker
                className="fixed z-[9999] min-w-[160px] overflow-hidden rounded-[var(--yu3-r-md)] border border-[var(--yu3-line)] bg-[var(--yu3-bg-surface)] text-[var(--yu3-ink)] shadow-[var(--yu3-shadow-md)]"
                style={{
                ...
                {VALID_PROJECT_ITEM_STATUSES.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3 py-1.5 text-[10px] font-semibold hover:bg-[var(--yu3-bg-surface-sunken)] transition-colors first:pt-2 last:pb-2 ${s === st ? `${scfg.color} font-bold` : "text-[var(--yu3-ink-faint)]"}`}
                    >
                      {scfg.label}
                    </button>
                  );
      ...
      {isYugo ? (
        <div className="flex items-center gap-2">
          <span className="text-[9px] font-semibold uppercase tracking-wide text-[var(--gold)]">
            Yugo Job
          </span>
          {linkedDelivery ? (
            <Link
        ...
        <div className="flex items-end gap-2">
          <div className="shrink-0" style={{ width: "5.5rem" }}>
            <label className="text-[9px] font-semibold uppercase tracking-wide text-purple-400 block mb-0.5">
              Carrier
            </label>
            <input
              value={carrier}
...
function VendorTrackerTab({
  ...
  const [groupBy, setGroupBy] = useState<"vendor" | "room" | "status">(
    "vendor",
  );
  const [viewMode, setViewMode] = useState<"compact" | "detailed">("compact");
  const [internalNote, setInternalNote] = useState(data.notes || "");
  const [savingNote, setSavingNote] = useState(false);
  const [noteTimeout, setNoteTimeout] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const getVendor = (item: InventoryItem) =>
    item.vendor_name || item.vendor || "No Vendor";

  const totalItems = inv.length;
  // Yugo flow stages (left → right, delivered = final)
  const stageOrdered = inv.filter((i) =>
    ["spec_selected", "ordered", "in_production"].includes(getStatus(i)),
  ).length;
  const stageReadyPickup = inv.filter(
    (i) => getStatus(i) === "ready_for_pickup",
  ).length;
  const stageInTransit = inv.filter((i) =>
    VT_TRANSIT.includes(getStatus(i)),
  ).length;
  const stageWarehouse = inv.filter((i) =>
    ["received_warehouse", "inspected", "stored"].includes(getStatus(i)),
  ).length;
  const stageScheduled = inv.filter(
    (i) => getStatus(i) === "scheduled_delivery",
  ).length;
  const stageDelivered = inv.filter((i) =>
    ["delivered", "installed"].includes(getStatus(i)),
  ).length;
  const stageUnassigned = inv.filter(
    (i) => !i.item_status || i.item_status === "pending",
  ).length;
  const issues = inv.filter((i) => getStatus(i) === "issue_reported").length;
  const viaCarrier = inv.filter(
    (i) =>
      i.handled_by === "other_carrier" ||
      i.vendor_delivery_method === "shipped_carrier",
  ).length;
  ...
  const groups: {
    ...
    groupBy === "vendor"
      ? Array.from(new Set(inv.map(getVendor))).map((name) => ({
          key: name,
          name,
          items: inv.filter((i) => getVendor(i) === name),
        }))
      : groupBy === "room"
        ? Array.from(
            new Set(inv.map((i) => i.room_destination || "No Room Assigned")),
          )
            .sort()
            .map((room) => ({
              key: room,
              name: room,
              items: inv.filter(
                (i) => (i.room_destination || "No Room Assigned") === room,
              ),
            }))
        : VT_STATUS_GROUPS.map((g) => ({
            key: g.key,
            name: g.label,
            items: inv.filter((i) =>
              g.statuses.includes(getStatus(i) as never),
            ),
            meta: g.color,
  ...
  return (
    <div className="divide-y divide-[var(--brd)]/50">
      ...
      <section className="py-5 first:pt-0">
        <div className="grid grid-cols-3 sm:grid-cols-7 gap-2">
          {[
            ...
            {
              label: "Delivered ✓",
              value: stageDelivered,
              color: "text-emerald-500",
            },
            {
              label: "At Warehouse",
              value: stageWarehouse,
              color: "text-teal-500",
            },
            {
              label: "Ready Pickup",
              value: stageReadyPickup,
              color: "text-amber-500",
            },
            {
              label: "In Transit",
              value: stageInTransit,
              color: "text-sky-500",
            },
            {
              label: "Via Carrier",
              value: viaCarrier,
              color: viaCarrier > 0 ? "text-purple-400" : "text-[var(--tx3)]",
            },
            {
              label: "Issues",
              value: issues,
              color: issues > 0 ? "text-red-500" : "text-[var(--tx3)]",
            },
          ].map(({ label, value, color }) => (
            <div key={label} className="text-center py-2">
              <div className={`text-[20px] font-bold ${color}`}>{value}</div>
              <div className="text-[9px] font-semibold uppercase tracking-wide text-[var(--tx3)] mt-0.5">
                {label}
              </div>
            </div>
          ))}
        ...
        <div className="py-2">
          {(() => {
            const stageWeight = (st: ProjectItemStatus) =>
              st === "delivered" || st === "installed"
                ? 100
                : st === "scheduled_delivery"
                  ? 85
                  : ["received_warehouse", "inspected", "stored"].includes(st)
                    ? 70
                    : st === "ready_for_pickup"
                      ? 45
                      : VT_TRANSIT.includes(st)
                        ? 55
                        : [
                              "spec_selected",
                              "ordered",
            ...
            const avgProgress =
              totalItems > 0
                ? inv.reduce((sum, i) => sum + stageWeight(getStatus(i)), 0) /
                  totalItems
                : 0;
            return (
              <>
                <div className="flex justify-between text-[10px] text-[var(--tx3)] mb-1.5">
                  <span className="font-semibold text-[var(--tx)]">
                    {stageDelivered} of {totalItems} delivered
                  </span>
      ...
      {viewMode === "compact" && (
        <section className="py-5">
          {inv.length === 0 ? (
            <p className="text-center py-10 text-[var(--tx3)] text-[13px]">
              No inventory items yet
            </p>
          ) : (
            ...
            {inv.length === 0 ? (
              ...
              <div className="space-y-3">
                {groups.map((group) => {
                  ...
                  const gReceived = group.items.filter((i) =>
                    VT_RECEIVED.includes(getStatus(i)),
                  ).length;
                  const gTransit = group.items.filter((i) =>
                    VT_TRANSIT.includes(getStatus(i)),
                  ).length;
                  const gIssues = group.items.filter(
                    (i) => getStatus(i) === "issue_reported",
                  ).length;
                  const contactItem =
                    groupBy === "vendor"
                      ? group.items.find(
                          (i) =>
                            i.vendor_contact_phone || i.vendor_contact_email,
                        )
                      : null;
                    ...
                    <div key={group.key} className="overflow-hidden">
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          <span
                            className={`text-[12px] font-bold uppercase tracking-wide ${group.meta || "text-[var(--tx)]"}`}
                          >
                            {group.name}
                          </span>
                        ...
                        <div className="pl-3 pt-1 space-y-2">
                          ...
                          {contactItem && (
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-3 py-2 text-[11px] text-[var(--tx3)]">
                              {contactItem.vendor_contact_name && (
                                <span className="font-semibold text-[var(--tx)]">
                                  {contactItem.vendor_contact_name}
                                </span>
                              )}
                              {contactItem.vendor_contact_phone && (
                                <a
                                  href={`tel:${contactItem.vendor_contact_phone}`}
                              ...
                              {contactItem.vendor_contact_email && (
                                <a
                                  href={`mailto:${contactItem.vendor_contact_email}`}
                                  className="text-[var(--gold)] hover:underline"
                                >
                                  {contactItem.vendor_contact_email}
                                </a>
                              )}
                              {contactItem.vendor_pickup_address && (
                                <span className="text-[var(--tx3)] flex items-center gap-1">
                                  <MapPin size={10} className="shrink-0" />
                                  {contactItem.vendor_pickup_address}
                                </span>
                              )}
                              {contactItem.vendor_pickup_window && (
                                <span className="text-[var(--tx3)] flex items-center gap-1">
          ...
          <section className="py-5 last:pb-0">
            <div className="py-3">
              <div className="flex items-center justify-between mb-2">
                ...
                {savingNote && (
                  <span className="text-[10px] text-[var(--tx3)]">Saving…</span>
                )}
              </div>
              <textarea
                value={internalNote}
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between text-[12px] py-0.5">
      ...
      <span className="text-[var(--tx)] font-medium text-right max-w-[65%]">
        {value}
      </span>
    </div>
  );
...
function StatCard({
  ...
  return (
    <div>
      <div className="text-[10px] font-bold tracking-wider uppercase text-[var(--tx3)]">
        {label}
      </div>
...
function TimelineIcon({ type }: { type: string }) {
  ...
  switch (type) {
    case "created":
      return (
        <div className={`${baseClass} bg-blue-500/10`}>
          <Plus {...iconProps} className="text-blue-500" />
        </div>
      );
    case "item_received":
    case "item_inspected":
      return (
        <div className={`${baseClass} bg-emerald-500/10`}>
          <CheckCircle2 {...iconProps} className="text-emerald-500" />
        </div>
      );
    case "phase_started":
    ...
    case "phase_added":
      return (
        <div className={`${baseClass} bg-purple-500/10`}>
          <CheckCircle2 {...iconProps} className="text-purple-500" />
        </div>
      );
    case "delivery_scheduled":
    case "delivery_completed":
      return (
        <div className={`${baseClass} bg-[var(--gold)]/10`}>
          <Truck {...iconProps} className="text-[var(--gold)]" />
        </div>
      );
    case "issue_flagged":
      return (
        <div className={`${baseClass} bg-red-500/10`}>
    ...
    case "invoice_generated":
      return (
        <div className={`${baseClass} bg-purple-500/10`}>
          <FileText {...iconProps} className="text-purple-500" />
        </div>
      );
    default:
      return (
        <div className={`${baseClass} bg-[var(--tx3)]/10`}>
          <Clock {...iconProps} className="text-[var(--tx3)]" />
        </div>
      );
  }
}
