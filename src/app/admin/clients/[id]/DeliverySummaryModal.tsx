  ...
  return (
    <ModalOverlay open={open} onClose={onClose} title="Project summary" maxWidth="md">
      <div className="p-5 space-y-3 text-[12px]">
        ...
        <Link href={getDeliveryDetailPath(delivery)} className="mt-4 inline-block w-full text-center py-2 rounded-lg text-[11px] font-semibold bg-[var(--admin-primary-fill)] text-[var(--btn-text-on-accent)] hover:bg-[var(--admin-primary-fill-hover)] transition-all">
          View full project →
        </Link>
      </div>
    </ModalOverlay>
