...
export default function TipsClient({
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-3 sm:px-5 md:px-6 py-4 sm:py-5 md:py-6 animate-fade-up min-w-0">
      <h1 className="font-heading text-h2-sm font-bold text-[var(--tx)] mb-1">Tips</h1>
      <p className="text-ui text-[var(--tx3)] mb-5">Crew gratuities from completed moves</p>

      <div className="mb-8">
        <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-3">Summary</div>
        <div className="grid grid-cols-3 gap-2">
          <div className="py-3">
            <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Total Tips</div>
            <span className="text-xl font-bold font-heading text-[var(--gold)]">{formatCurrency(totalTips)}</span>
          </div>
          <div className="py-3">
            <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Average</div>
            <span className="text-xl font-bold font-heading text-[var(--tx)]">{formatCurrency(avgTip)}</span>
          </div>
          <div className="py-3">
            <div className="text-label font-semibold tracking-wider uppercase text-[var(--tx3)] mb-1">Total Count</div>
      ...
      <div className="border-t border-[var(--brd)]/30 pt-6">
        <div className="text-section font-bold tracking-[0.14em] uppercase text-[var(--tx3)]/50 mb-3">Recent tips</div>
        <DataTable<Tip>
          data={tips}
          columns={columns}
          keyField="id"
