...
const AGREEMENT_HEADER: Record<string, { title: string; subtitle: string }> = {
  ...
  office_move: {
    ...
    subtitle:
      "Please review and sign to continue your relocation on the terms we quoted.",
  },
  single_item: {
  ...
  b2b_oneoff: {
    ...
    subtitle:
      "Please review and sign to confirm this delivery on the terms quoted.",
  },
  b2b_delivery: {
    title: "Commercial Delivery Agreement",
    subtitle:
...
export default function ContractSign({
  ...
  const isLogisticsDeliveryCopy = isClientLogisticsDeliveryServiceType(
    q.serviceType,
  );
  ...
  const agreementMeta =
    ...
    (isClientLogisticsDeliveryServiceType(q.serviceType)
      ...
      : {
          ...
          subtitle:
            "Please review and sign to continue on the terms we quoted.",
        });
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setError(
          data.error ?? "Failed to process signature. Please try again.",
        );
        setSigning(false);
        return;
      }
    ...
    >
      ...
      <div className="pt-2 border-t" style={{ borderColor: `${FOREST}12` }}>
        <p className="text-[11px] leading-snug" style={{ color: FOREST_MUTED }}>
          This agreement is binding under the Ontario Electronic Commerce Act,
          2000. A typed signature with agreement checkbox, timestamp, and IP
          address constitutes a valid electronic signature. For our complete
          Terms of Service and Privacy Policy, visit{" "}
          <a
  ...
  return (
    <div className={onWineShell ? "space-y-5" : "space-y-6"}>
      ...
      {/* ─── Pricing section ─── */}
      <div
        className={agreementTheme.receiptClass}
        style={{ color: agreementTheme.ink }}
      >
        <div
          className="h-1 w-14 mb-4 rounded-sm shrink-0"
          style={{ backgroundColor: agreementTheme.accentBar }}
          aria-hidden
        />
        <p
          className={`${QUOTE_EYEBROW_CLASS} mb-3`}
          style={{ color: agreementTheme.sectionEyebrow }}
        >
          Investment summary
        </p>
            ...
            <>
              {q.addons.map((a, i) => (
                <div
                  ...
                  style={{
                    borderLeft: `2px solid ${agreementTheme.addonRail}`,
                  }}
                >
                  <span
                    className="text-[11px]"
        ...
        <div
          className="flex justify-between items-baseline gap-4 pt-3 mt-3 border-t"
          style={{ borderColor: agreementTheme.borderHair }}
        >
          <span
        ...
        <div className="mt-4 flex flex-col gap-2">
          ...
          {paidInFullAtBooking && isLogisticsDeliveryCopy && !isBinRental && (
            <p
              ...
              style={{ color: agreementTheme.inkBody }}
            >
              Full payment (incl. HST) is collected after you sign to confirm
              this delivery booking.
            </p>
          )}
        </div>
      </div>
      ...
      >
        <p
          ...
          style={{ color: agreementTheme.signEyebrow }}
        >
          Your signature
        </p>
          ...
          <button
            ...
            style={{
              backgroundColor: canSign
                ? agreementTheme.signCta
                : agreementTheme.signCtaDisabled,
            }}
          >
            {signing ? (
        ...
        <p
          ...
          style={{ color: agreementTheme.legalFooter }}
        >
          Your electronic signature is legally binding under the Ontario
