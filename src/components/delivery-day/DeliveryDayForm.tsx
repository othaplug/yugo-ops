      ...
      <div className="flex gap-1.5">
        {stepLabels.map((label, i) => (
          <div key={i} className="flex-1">
            ...
            <span className={`text-[10px] mt-1 block ${i === step - 1 ? "text-[var(--accent-text)] font-semibold" : "text-[var(--tx3)]"}`}>{label}</span>
          </div>
        ))}
      </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {TIME_WINDOW_CHOICES.map((tw) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-5 h-5 mx-auto mb-1 ${active ? "text-[var(--accent-text)]" : "text-[var(--tx3)]"}`} />
      ...
      {step === 2 && (
        <div className="space-y-5">
            ...
            <div key={stop.id} className="rounded-xl border border-[var(--brd)] bg-[var(--card)] p-4 space-y-3 shadow-sm">
              <div className="flex items-center justify-between">
                <h4 className="text-[13px] font-bold text-[var(--tx)] flex items-center gap-1.5">
                  <span className="admin-btn admin-btn-sm admin-btn-primary">{idx + 1}</span>
                  Stop {idx + 1}
                </h4>
              ...
              {availableServices.filter((s) => s.price_unit !== "percentage").length > 0 && (
                <div className="space-y-1.5">
                  <span className="text-[9px] font-semibold tracking-wider uppercase text-[var(--tx3)]">Services at this stop</span>
                  {availableServices
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <input
                            ...
                            className="rounded border-[var(--brd)] text-[var(--accent-text)] focus:ring-[var(--gold)] shrink-0"
                          />
          ...
          <button
            type="button"
            onClick={addStop}
            className="w-full py-3 rounded-xl text-[13px] font-semibold text-[var(--tx2)] hover:text-[var(--accent-text)] transition-colors flex items-center justify-center gap-2"
          >
        ...
        <div className="space-y-5">
          <div className="rounded-xl border-2 border-[var(--gold)]/30 bg-[var(--gdim)] p-5 space-y-4">
            ...
            <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-gradient-to-r from-[var(--gold)]/12 to-[var(--gold)]/8 border border-[var(--gold)]/20">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-[var(--gold)]/15 text-[var(--accent-text)]">
            ...
            {pricing && (
              <div className="space-y-2 pt-2 border-t border-[var(--gold)]/20">
                ...
                <div className="border-t border-[var(--gold)]/20 pt-2 mt-1 flex justify-between">
                  <span className="text-[length:var(--text-base)] font-bold text-[var(--tx)]">Total</span>
                  <span className="text-[18px] font-bold text-[var(--accent-text)]">{fmt(pricing.totalPrice)}</span>
                </div>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {VEHICLE_OPTIONS.map((v) => (
                  <button
                    ...
                    className={`px-2 py-2 rounded-lg text-[11px] font-semibold border transition-all ${selectedVehicle === v.value ? "border-[var(--gold)] bg-[var(--gold)]/10 text-[var(--accent-text)]" : "border-[var(--brd)] text-[var(--tx2)] hover:border-[var(--gold)]/50"}`}
                  >
                    {v.short}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                {(["full_day", "half_day"] as const).map((dt) => (
                  <button
                    ...
                    className={`px-3 py-2.5 rounded-lg text-[12px] font-semibold border transition-all ${selectedDayType === dt ? "border-[var(--gold)] bg-[var(--gold)]/10 text-[var(--accent-text)]" : "border-[var(--brd)] text-[var(--tx2)] hover:border-[var(--gold)]/50"}`}
                  >
          ...
          {stops.map((stop, idx) => (
            <div key={stop.id} className="rounded-xl border border-[var(--brd)] p-4 space-y-1.5">
              <h4 className="text-[12px] font-bold text-[var(--tx)] flex items-center gap-1.5">
                <span className="admin-btn admin-btn-primary">{idx + 1}</span>
                Stop {idx + 1}
              </h4>
