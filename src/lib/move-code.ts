...
export function getMoveCode(
  ...
  const code =
    typeof move === "object" &&
    move &&
    ("move_code" in move ? move.move_code : move.move_number)
      ? (move as { move_code?: string; move_number?: string }).move_code ||
...
export function getMoveDetailPath(
  move: { move_code?: string | null; id?: string } | null | undefined,
): string {
...
export function getDeliveryDetailPath(
  delivery: { delivery_number?: string | null; id?: string } | null | undefined,
): string {
...
export function getTrackMoveSlug(move: {
  move_code?: string | null;
  id: string;
}): string {
  const code = move.move_code?.trim().replace(/^#/, "");
...
export function getTrackDeliverySlug(delivery: {
