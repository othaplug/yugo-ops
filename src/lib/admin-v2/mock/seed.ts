...
export const FIRST_NAMES = [
  ...
  "Abigail", "Samuel", "Jacob", "Leah", "Noah", "Chloe",
]
...
const ADDR_STREETS = [
  "Park Ave", "Lexington Ave", "Columbus Ave", "Madison Ave", "Amsterdam Ave",
  "Broadway", "Canal St", "Hudson St", "Wall St", "Houston St",
]
...
const phone = (r: () => number) => {
  ...
  return `+1 (212) ${Math.floor(n / 10000)}-${n % 10000}`.replace(/(\d{3})(\d+)/, "$1 $2")
}
...
export const generateInvoices = (
  ...
  return moves.slice(0, 140).map((move, index) => {
    ...
    return {
      ...
      total,
      dueAt: new Date(
        new Date(createdAt).getTime() + 14 * DAY,
      ).toISOString(),
      paidAt: status === "paid" ? new Date(now - r() * 30 * DAY).toISOString() : null,
      createdAt,
    }
  })
}
...
export const generateBuildings = (
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      lastMoveAt:
        r() > 0.2
          ? new Date(now - r() * 120 * DAY).toISOString()
          : null,
    }
  })
}
