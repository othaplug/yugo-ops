...
export async function maybeNotifyB2BOneOffOutForDelivery(
  ...
  const { data: d } = await admin
    .from("deliveries")
    .select(
      "id, booking_type, organization_id, contact_email, business_name, customer_name, delivery_address, tracking_token, b2b_business_notify_en_route_sent_at"
    )
    .eq("id", deliveryId)
    .maybeSingle();

  if (!d || !isB2BOneOffRow(d) || d.b2b_business_notify_en_route_sent_at) return;
  if (!d.tracking_token || !d.contact_email) return;
...
<div style="padding:40px 24px;font-family:system-ui,sans-serif;color:#FAF7F2;max-width:560px;margin:0 auto">
  ...
  await sendEmail({ to: d.contact_email!, subject: subj, html: inner }).catch(() => {});
  await admin
    .from("deliveries")
...
export async function maybeNotifyB2BOneOffDelivered(deliveryId: string): Promise<void> {
  ...
  const { data: d } = await admin
    .from("deliveries")
    .select(
      "id, booking_type, organization_id, contact_email, business_name, customer_name, tracking_token, b2b_business_notify_delivered_sent_at"
    )
    .eq("id", deliveryId)
    .maybeSingle();
