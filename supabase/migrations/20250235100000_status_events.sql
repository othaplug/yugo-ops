...
DROP POLICY IF EXISTS "Authenticated can read status_events" ON public.status_events;
CREATE POLICY "Authenticated can read status_events"
  ON public.status_events FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Service role can insert status_events" ON public.status_events;
